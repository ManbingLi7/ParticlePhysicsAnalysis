...
def calc_ratio_and_err(a, b, delta_a, delta_b, correlation=None):
    ratio = a/b
    if correlation is None:
        err = a/b * np.sqrt((delta_a / a)**2 + (delta_b / b)**2)
    else:
        err = ratio * np.sqrt((delta_a / a)**2 + (delta_b / b)**2 - 2 * delta_a * delta_b * correlation / (a * b))
    return ratio, err
...
def calc_rig_iso(beta, iso):
    return ISOTOPES_MASS[iso] * beta /(ISOTOPES_CHARGE[iso] * np.sqrt((1 - beta**2)))
