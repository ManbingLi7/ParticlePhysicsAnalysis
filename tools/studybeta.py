...
from .calculator import calc_beta, calc_ekin_from_beta
import pickle
...
def GetMCRICHTunedBeta_WithHighR(events, beta, dec, iso, exterr='0', given_korr_sigma =None):
    ...
    print(dec, 'mean_shift:', mean_shift, '  sigma_scale:', sigma_scale)    
    ...
    print(dec, 'sigma_scale:', sigma_scale, 'mean_shift', mean_shift)
    return beta_tuned
############################################################
def GetMCRICHTunedInverseBeta_GivenTuneValues(events, beta, iso, meanshift, scalesigma):
    true_rigidity_atRICH = ak.to_numpy(events["mevmom1"][:, 17]/ISOTOPES_CHARGE[iso])      
    true_beta_atRICH = calc_beta(true_rigidity_atRICH, ISOTOPES_MASS[iso], ISOTOPES_CHARGE[iso]) 
    inverse_beta_tuned = (1/beta - 1/true_beta_atRICH) * scalesigma + 1/true_beta_atRICH + meanshift
    print('sigma_scale:', scalesigma, 'mean_shift', meanshift)
    return 1/inverse_beta_tuned
...
def ShiftToFBeta(events):
    fitlow = 0.3
    fitup = 10
    beta = events.tof_betah
    ekn = np.array(calc_ekin_from_beta(beta))
    ekn[ekn<0.3] = 0.3
    ekn[ekn>10] = 10
    shiftpar = np.array([-0.00230416, 0.00414733, -0.00194352])
    shift = shiftpar[0] + shiftpar[1]*np.log(ekn) + shiftpar[2]*np.log(ekn)*np.log(ekn)
    new_beta = 1/(1/beta + shift)
