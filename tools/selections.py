...
from .calculator import calc_rig, calc_ekin_from_beta, calc_beta
...
####should the L1 lower charge cut be tighter?
def SelectUnbiasL1LowerCut(events, zsel):
    events = events[events.tk_exqln[:, 0, 0, 0] > 0]
    events = events[events.tk_exqln[:, 0, 0, 1] > 0]
    events = events[(events.tk_exqls[:, 0] & 0x10013D)==0]
    events = events[events.tk_exqln[:, 0, 0, 2] > (zsel - 0.4)]
    #selection = events.tk_exqln[:, 0, 0, 2] > (zsel - 0.46- (zsel-3)*0.16)
    return events

def SelectUnbiasL1Q(events, zsel):
    events = events[events.tk_exqln[:, 0, 0, 0] > 0]
    events = events[events.tk_exqln[:, 0, 0, 1] > 0]
    events = events[(events.tk_exqls[:, 0] & 0x10013D)==0]
    events = events[events.tk_exqln[:, 0, 0, 2] > (zsel - 0.4)]
    events = events[events.tk_exqln[:, 0, 0, 2] < (zsel + 0.7)]
    #selection = events.tk_exqln[:, 0, 0, 2] > (zsel - 0.46- (zsel-3)*0.16)
    return events
...
#Cutoff Selection
#######################################
def geomagnetic_IGRF_cutoff(events, factor=1.2, datatype='ISS'):
    ...
    if datatype == 'ISS':
        ...
        selection = binlowedge  > factor * events.cal_igrf
    return events[selection]

def geomagnetic_GenMCRig_Cutoff(events, nuclei, factor=1.0):
    rig_gen = ak.to_numpy(events.mmom/NUCLEI_CHARGE[nuclei])
    selection = rig_gen  > factor * events.cal_igrf
    return events[selection]



def geomagnetic_cutoff_Li7(events, dec, datatype='ISS', factor=1.0, englos_factor=1.0):
    ...
    #df_probpars_TofLi6 = np.load('/home/manbing/Documents/Data/data_LiP8/unfold/Tof_mean_polypar_inversebeta_residual.npz')
    ...
    df_probpars_AglLi7 = np.load('/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/LiBeta/AglLi7_polypar_inversebeta.npz')
    df_probpars_NaFLi7 = np.load('/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/LiBeta/NaFLi7_polypar_inversebeta.npz')
    df_probpars_TofLi7 = np.load('/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/LiBeta/TofLi7_polypar_inversebeta.npz')
    #df_probpars_TofLi7 = np.load('/home/manbing/Documents/Data/data_LiP8/unfold/Tof_mean_polypar_inversebeta_residual.npz')

    mass_nucleon_gev = 0.9314941 #in GeV 
    df_probparsLi7 = {'Tof': df_probpars_TofLi7, 'NaF': df_probpars_NaFLi7, 'Agl': df_probpars_AglLi7}
    df_probparsLi6 = {'Tof': df_probpars_TofLi6, 'NaF': df_probpars_NaFLi6, 'Agl': df_probpars_AglLi6}
        
    
    rigiditycutoff = events.cal_igrf
    
    betacutoffLi6  = calc_beta(rigiditycutoff, ISOTOPES_MASS['Li6'], NUCLEI_CHARGE['Li'])
    betacutoffLi7  = calc_beta(rigiditycutoff, ISOTOPES_MASS['Li7'], NUCLEI_CHARGE['Li'])
    ...
    if dec == 'Tof':
        deltabetaLi6 = np.array(np.poly1d(df_probparsLi6[dec]['mean'])(np.log(beta))) #+ np.array(np.poly1d(df_probparsLi6[dec]['sigma'])(np.log(beta)))
        deltabetaLi7 = np.array(np.poly1d(df_probparsLi7[dec]['mean'])(np.log(beta))) #- np.array(np.poly1d(df_probparsLi7[dec]['sigma'])(np.log(beta)))
    else:
        deltabetaLi6 = np.array(np.poly1d(df_probparsLi6[dec]['mean'])(np.log(beta))) #+ np.array(np.poly1d(df_probparsLi6[dec]['sigma'])(np.log(beta)))
        deltabetaLi7 = np.array(np.poly1d(df_probparsLi7[dec]['mean'])(np.log(beta))) #- np.array(np.poly1d(df_probparsLi7[dec]['sigma'])(np.log(beta)))
    #deltabetaLi6[deltabetaLi6 < 0] = 0.0000000000000001
    #deltabetaLi7[deltabetaLi7 < 0] = 0.0000000000000001
    beta_toiLi6 = beta + deltabetaLi6 * englos_factor
    beta_toiLi7 = beta + deltabetaLi7 * englos_factor

    print(factor, 1/factor)
    
    #betabinning = Binning(fbinning_beta())
    #binIndex = betabinning.get_indices(beta_toi)  
    #binlowedge = betabinning.edges[binIndex]

    selection = (beta_toiLi7 > factor * betacutoffLi7) & (beta_toiLi6 < 1/factor * betacutoffLi6)
    return events[selection]

def geomagnetic_cutoff_Li7MC(events, dec, datatype='MC', factor=1.0, englos_factor=1.0):
    #read the resolution fit parameters
    ...
    rig_gen = ak.to_numpy(events.mmom/NUCLEI_CHARGE['Li'])
    beta_mcgen = calc_beta(rig_gen, ISOTOPES_MASS['Li7'], NUCLEI_CHARGE['Li'])
    print(factor, 1/factor)
    ...
    selection = (beta_mcgen > factor * betacutoffLi7) & (beta_mcgen < 1/factor * betacutoffLi6)
    return events[selection]


def geomagnetic_cutoff_Be10(events, dec, datatype='ISS', factor=1.0):
    df_probpars_AglBe7 = np.load('/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/beta/AglBe7_polypar_inversebetav2.npz')
    df_probpars_NaFBe7 = np.load('/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/beta/NaFBe7_polypar_inversebetav2.npz')
    df_probpars_TofBe7 = np.load('/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/beta/TofBe7_polypar_inversebetav2.npz')
    mass_nucleon_gev = 0.9314941 #in GeV
    ...
    betacutoffBe9  = calc_beta(rigiditycutoff, ISOTOPES_MASS['Be9'], NUCLEI_CHARGE['Be'])
    betacutoffBe10  = calc_beta(rigiditycutoff, ISOTOPES_MASS['Be10'], NUCLEI_CHARGE['Be'])

    if (datatype == 'ISS') & (dec == 'Tof'):
        beta = events.tof_betah
    elif (datatype == 'MC') & (dec == 'Tof'):
        beta = events.tof_betahmc
    else:
        beta = events.rich_beta[:, 0]

    deltabeta = np.poly1d(df_probpars[dec]['mean'])(np.log(beta))
    beta_toi = beta + deltabeta 
    selection = (beta_toi > factor * betacutoffBe10) & (beta_toi < 1/factor * betacutoffBe9)
    return events[selection]
...
def geomagnetic_cutoff_richcmat(events, nuclei, ana_isotope, rebin):
    ...
    binIndex = betabinning.get_indices(beta) 
    binlowedge = betabinning.edges[binIndex]
    iso_lowest_A = ISOTOPES[nuclei][0]
    bin_low_rig = calc_rig(binlowedge, ISOTOPES_MASS[iso_lowest_A], NUCLEI_CHARGE[nuclei]) 
    factor = CUTOFF_FACTOR[ana_isotope]    
    rigiditycutoff = (events.mcutoffi)[:, 1, 1]      
    selection = bin_low_rig > factor*rigiditycutoff         
    return events[selection]
...
def CutTofRichBetaConsistency(events, lim, datatype, useToFMCOldVersion=False):
    #richbeta = events.rich_beta_cor
    richbeta = events.rich_beta[:, 0]
    if useToFMCOldVersion is True:
...
def selector_tof(events, nuclei, isotope, datatype, cutoff=True, rebin=True, cut_ubl1=True, cutTofEdge=False):
...
def selector_naf_ciematvar(events, nuclei, isotope, datatype, cutoff=True, rebin=True, cut_ubl1=True, cutTofEdge=False):
    qsel = NUCLEI_CHARGE[nuclei]
    events = SelectCleanEvent(events)
    #events = SelectUnbiasL1LowerCut(events, qsel)   
...
def selector_agl_ciematvar(events, nuclei, isotope, datatype, cutoff=True, rebin=True, cut_ubl1=True, cutTofEdge=False):
    qsel = NUCLEI_CHARGE[nuclei]
    events = SelectCleanEvent(events)
    #events = SelectUnbiasL1LowerCut(events, qsel)   
    events = Select_HasCIEMATRing(events)
