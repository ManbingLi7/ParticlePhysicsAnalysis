...
rigidity_binning = kinetic_energy_neculeon_binning()
xbinning = Binning(kinetic_energy_neculeon_binning())
...
def main():
    ...
    df_unfold = np.load(os.path.join(args.resultdir, 'beta_resolution.npz'))
    hist_response = WeightedHistogram.from_file(df_unfold, 'hist_response_NaF_Be7')
    hist_datacounts = WeightedHistogram.from_file(df_unfold, 'histRec_NaF_Be7')
    hist_mcTrueRig = WeightedHistogram.from_file(df_unfold, 'histTrue_NaF_Be7')
    hist_mcRecRig = WeightedHistogram.from_file(df_unfold, 'histRec_NaF_Be7')    
    ...
    unfolded_results = iterative_unfold(data=hist_mcRecRig.values[1:-1], data_err=hist_mcRecRig.get_errors()[1:-1], response=normalized_response, response_err = response_err, efficiencies = effi, efficiencies_err = effi_err, callbacks = [Logger()])
