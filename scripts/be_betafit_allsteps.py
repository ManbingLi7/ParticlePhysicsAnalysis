...
from tools.massfit_tools import get_fitpdf_witherr, get_fitpdferrorband, get_unp_pars_binbybin, plot_fitmc_pars, plot_fitdata_pars, plot_fitmc_compare_isopars, plot_fitMCISO_pars, plot_fitmc_compare_isopars_betareso_Be, plot_fitmc_pars_betareso, savegraphpar
...
mcpars_initlims = {'Tof': {'mean': (-0.03, 0.005), 'sigma': (0.005, 0.015), 'fraccore': (0.9, 1.0), 'sigma_ratio':(1.0, 10.0), 'asy_factor':(1.0, 10.0)},
                   'NaF': {'mean': (-0.02, 0.001), 'sigma': (0.0001, 0.01), 'fraccore': (0.8, 1.0), 'sigma_ratio':(1.5, 3.0), 'asy_factor':(0.9, 1.25)},
                   'Agl': {'mean': (-0.005, 0.001), 'sigma': (0.00005, 0.005), 'fraccore': (0.8, 1.0), 'sigma_ratio':(1.4, 3.0), 'asy_factor':(1.0, 1.2)}}
...
detectors_energyrange = {"Tof": [1.6, 3.0], "NaF": [2.2, 4.0], "Agl": [3.8, 15]}
...
def main():
    ...
    parser.add_argument("--filename_mc", default="/home/manbing/Documents/Data/data_LiP8/Hist2D/LiMC_beta_resolution_untunedBeta.npz",  help="Path to root file to read tree from")
    parser.add_argument("--plotdir", default="plots/LiP8/BetaReso", help="Directory to store plots and result files in.")
    ...
    fit_range = {"Tof": [-0.05, 0.04], "NaF":[-0.01, 0.007], "Agl": [-0.003, 0.002]}
    ...
    with np.load(args.filename_mc) as mc_file:
        for dec in detectors:
            for iso in ISOTOPES[nuclei]:
                hist2d_mass_energy_mc[dec][iso] = WeightedHistogram.from_file(mc_file, f"beta_resolution_{dec}_{iso}")
    ...
    for dec in detectors:
        massfit_mciso[dec] = MassFunctionFit[dec]([MCISO], hist2d_mass_energy_mc[dec][MCISO], detectors_energyrange[dec], fit_range[dec], dec, is_constraint=False)
    ...
    for dec in detectors:
        massfit_mciso_iter1[dec] = MassFunctionFit[dec]([MCISO], hist2d_mass_energy_mc[dec][MCISO], detectors_energyrange[dec], fit_range[dec], dec, is_constraint=False)
    ...
    for dec in detectors:
        for iso in isotopes:
            massfit_mciso_iter2[iso][dec] = MassFunctionFit[dec]([iso], hist2d_mass_energy_mc[dec][iso], detectors_energyrange[dec], fit_range[dec], dec, is_constraint=False)
    ...
    plt.close()
    for name in par_initial[dec].keys():
        savegraphpar(name, detectors, dict_fit_results, massfit, nuclei, isotopes)
