...
def update_guess_binbybinfit_initial(massfits, graph_pars, isotopes):
    guess_binbybin = {}
...
def main():
    ...
    parser.add_argument("--plotdir", default="plots/BeP8/fitData_P811yr_rebin_notTofEdgeSysErr", help="Directory to store plots and result files in.")
    ...
    parser.add_argument("--niter", default=100, type=int, help="choose if is_nonlinearconstraint")
    ...
    fixed_pvalues = {'Tof': ['sigma', 'asy_factor', 'sigma_ratio', 'fraccore'],
                     'NaF': ['sigma', 'asy_factor', 'sigma_ratio', 'fraccore'],
                     'Agl': ['sigma', 'asy_factor', 'sigma_ratio', 'fraccore']}
    ...
    filedir_fitdatapars = '/home/manbing/Documents/lithiumanalysis/scripts/plots/BeP8/fitData_P811yr_rebin_notTofEdgeFixP'
    graph_fitpars = {dec: {} for dec in detectors}
    for par in par_names.keys():
        npzfile = np.load(os.path.join(filedir_fitdatapars, f'graph_fitpar_{par}.npz'))
        for dec in detectors:
    ...
    for iiter in range(args.niter):
        for dec in detectors:
            ...
            guess_initial[dec] = update_guess_binbybinfit_initial(massfit[iiter][dec], graph_fitpars[dec], isotopes)
    ...
    for iiter in range(args.niter):
        for dec in detectors:
            ...
            for iso, isonum in enumerate(isotopes_atom_num):
                ...
                graph_counts_iso[iiter][dec][isoname].add_to_file(dict_graph_counts, f'graph_counts_{dec}_{isonum}_{iiter}')
    ...
    if args.isconstraint: 
        for iiter in OptimizedDataSample:
            for dec in detectors:
                counts_be10[iiter][dec] = massfit[iiter][dec].get_data_infitrange().sum(axis=1) - fit_norm[iiter][dec][7] - fit_norm[iiter][dec][9]
                yerrbe10 = np.sqrt(graph_counts_iso[iiter][dec]["Be7"].yerrs ** 2 + graph_counts_iso[iiter][dec]["Be9"].yerrs ** 2 - 2 * 0.8*abs(graph_counts_iso[iiter][dec]["Be7"].yerrs * graph_counts_iso[iiter][dec]["Be9"].yerrs))
                graph_counts_iso[iiter][dec]["Be10"] = MGraph(massfit[iiter][dec].x_fit_energy, counts_be10[iiter][dec], yerrbe10)     
                graph_counts_iso[iiter][dec]["Be10"].add_to_file(dict_graph_counts, f'graph_counts_{dec}_Be10_{iiter}')
