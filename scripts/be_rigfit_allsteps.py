...
par_names = {"mean": 0.001, "sigma": 0.1, "fraccore":0.84, "sigma_ratio":1.3, "asy_factor":1.4}

scale_factor = {"Be7": 1, "Be9": 7./9., "Be10": 7.0/10.0}
mean_scale_factor_fromMC = {"Be7": 1, "Be9": 0.77949789, "Be10": 0.70255866}

mcpars_initlims = {'Tof': {'mean': (-0.01, 0.02), 'sigma': (0.1, 0.13), 'fraccore': (0.75, 1.0), 'sigma_ratio':(1.0, 10.0), 'asy_factor':(1.0, 2.0)},
                   'NaF': {'mean': (-0.01, 0.02), 'sigma': (0.008, 0.03), 'fraccore': (0.89, 0.91), 'sigma_ratio':(1.5, 2.0), 'asy_factor':(1.2, 1.25)},
                   'Agl': {'mean': (-0.01, 0.02), 'sigma': (0.008, 0.03), 'fraccore': (0.78, 0.9), 'sigma_ratio':(1.4, 2.0), 'asy_factor':(1.1, 1.2)}}
...
detectors_energyrange = {"Tof": [4.0, 20], "NaF": [1.0, 4.0], "Agl": [3.8, 11]} #Agl: 4.0, 10
#detectors_energyrange = {"Tof": [1.92, 40], "NaF": [1.3, 3.4], "Agl": [4.0, 11.5]}
fit_range_mass_nuclei = [0.05, 0.25]
...
polyfitp0["Agl"] = {"mean": [0.0, 0.0, 0.0], "sigma":[0.1, 0.0, 0.0, 0.001], 'sigma_ratio': [1.5, 0.1, 0.1], 'asy_factor':[1.1, 0.1, 0.01], 'fraccore': [0.75]}
polyfitp0["NaF"] = {"mean": [0.0, 0.0, 0.0], "sigma":[0.1, 0.0, 0.0], 'sigma_ratio': [1.5], 'asy_factor':[1.1], 'fraccore': [0.75]}
polyfitp0["Tof"] = {"mean": [0.0, 0.0, 0.0], "sigma":[0.1, 0.0, 0.0, 0.001], 'sigma_ratio': [1.2, 0.1, 0.1], 'asy_factor':[1.4, 0.01, 0.001], 'fraccore': [0.823]}
...
def main():
    ...
    for dec in detectors:
        ...
        massfit_mciso[dec].draw_fit_results_mc(resultpars_mciso[dec], dict_results_mciso[dec], args.plotdir, fit_simultaneous=False, guess=None, x_label=r"$\mathrm{(1/R_{meas} - 1/R_{gen})/1/R_{gen}}$", figname='iter0')
    ...
    for dec in detectors:
        for iso in isotopes:
            massfit_mciso_iter2[iso][dec] = MassFunctionFit[dec]([iso], hist2d_mass_energy_mc[iso], detectors_energyrange[dec], fit_range[iso], dec, False)
            ...
            fixpars_mciso_iter2 = ['fraccore', 'sigma_ratio']

            resultpars_mciso_iter2[iso][dec], dict_results_mciso_iter2[iso][dec] = massfit_mciso_iter2[iso][dec].perform_fit(guess_mciso_iter2[iso][dec], lim_pars=limpars_mciso_iter2[dec], fixed_pars=fixpars_mciso_iter2, fit_simultaneous=False)
            massfit_mciso_iter2[iso][dec].draw_fit_results_mc(resultpars_mciso_iter2[iso][dec], dict_results_mciso_iter2[iso][dec], MCISO_PLOTDIR, fit_simultaneous=False, guess=None, x_label=r"$\mathrm{(1/R_{meas} - 1/R_{gen})/1/R_{gen}}$", figname='iter2')
    ...
    plt.close()
    for name in par_names.keys():
        plot_fitmc_compare_isopars_rigreso(name, detectors, dict_results_mciso_iter2, massfit_mciso_iter2, args.plotdir, polyfitp0, par_names_axes, ylim_range_be7, nuclei, isotopes, legend='iter2')
