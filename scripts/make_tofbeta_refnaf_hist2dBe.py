...
from tools.binnings_collection import LithiumRichAglBetaResolutionBinning, LithiumRigidityBinningFullRange, fbinning_energy, kinetic_energy_neculeon_binning, Rigidity_Analysis_Binning
...
isoweight = {'Be7': 0.6, 'Be9': 0.3, 'Be10': 0.1,
             'O16': 1.0, 'C12': 1.0, 'Li6': 0.5, 'Li7':0.5}
...
def handle_file(arg):    
    ...
    for dec in detectors: 
        for events in read_tree(filename_iss, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
            ...
            events = rich_selectors["CIEMAT"][decref](events, nuclei, iso, "ISS", cutoff=False)
            ...
            hist_issbetareso[dec].fill(ekin_rich, (1/beta_tof-1/beta_rich), weights=np.ones_like(beta_tof))
    ...
    for dec in detectors: 
        for i_iso, iso in enumerate(isotopes):
            for events in read_tree(filename_mc[i_iso], treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
                ...
                events = rich_selectors["CIEMAT"][decref](events, nuclei, iso, "MC", cutoff=False)
                ...
                true_momentum_atRICH = ak.to_numpy(events["mevmom1"][:, 17])
                true_rigidity_atRICH = ak.to_numpy(events["mevmom1"][:, 17]/NUCLEI_CHARGE[nuclei])
                true_beta_atRICH = calc_beta(true_rigidity_atRICH, ISOTOPES_MASS[iso], ISOTOPES_CHARGE[iso])
                true_ekin_atRICH = calc_ekin_from_rigidity_iso(true_rigidity_atRICH, iso)

                true_momentum_atInn = ak.to_numpy(events["mevmom1"][:, 12])
                true_rigidity_atInn = ak.to_numpy(events["mevmom1"][:, 12]/NUCLEI_CHARGE[nuclei])
                true_beta_atInn = calc_beta(true_rigidity_atInn, ISOTOPES_MASS[iso], ISOTOPES_CHARGE[iso])
                true_ekin_atInn = calc_ekin_from_rigidity_iso(true_rigidity_atInn, iso)

                beta_tof = ak.to_numpy(events['tof_betah'])
                beta_rich = ak.to_numpy(events['rich_beta_cor'])
                #beta_naf = ak.to_numpy(events['rich_beta'][:, 0])
                
                beta_rich_tuned = GetMCRICHTunedBeta_WithHighR(events, beta_rich, decref, iso)
                
                ekin_rich = calc_ekin_from_beta(beta_rich)
                ekin_rich_tuned = calc_ekin_from_beta(beta_rich_tuned)  

                weight_mix = isoweight[iso] * mc_weight
                if usetunedRICH is True:
                    refbeta = beta_rich_tuned
                    refEkin = ekin_rich_tuned
                else:
                    refbeta = beta_rich
                    refEkin = ekin_rich
                    
                hist_mcbetareso[dec][iso].fill(refEkin, 1/beta_tof - 1/refbeta, weights=mc_weight)
                hist_mcbetareso_mix[dec].fill(refEkin, 1/beta_tof - 1/refbeta, weights=weight_mix)

                #hist_mcbetareso[dec][iso].fill(true_ekin_atRICH, beta_tof - true_beta_atRICH, weights=mc_weight)
                #hist_mcbetareso_mix[dec].fill(true_ekin_atRICH, beta_tof - true_beta_atRICH, weights=weight_mix)
...
def main():
    ...
    parser.add_argument("--filename_iss", default= "/home/manbing/Documents/Data/data_LiP8/rootfile/LiISS_P8_CIEBeta.root",  help="(e.g. results/ExampleAnalysisTree*.root)")
    ...
    args.filename_mc = [f"/home/manbing/Documents/Data/data_{nuclei}P8/rootfile/{iso}MC_B1308.root" for iso in isotopes]
    ...
    np.savez(os.path.join(args.resultdir, f"{nuclei}_tofbetaResidual_refUntuned{decref}_B1308mcweight_inversebeta.npz"), **result_resolution)
