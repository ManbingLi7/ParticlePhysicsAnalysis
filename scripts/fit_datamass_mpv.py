...
from tools.constants import NUCLEI_CHARGE, ISOTOPES_MASS, ISOTOPES_COLOR, ISO_LABELS, NUCLEI_NUMBER
...
ANALYSIS_RANGE_EKIN = {"Tof": [0.4, 1.1], "NaF": [1.1, 4.0], "Agl": [4.0, 9.0]}  
...
def main():
    ...
    #parser.add_argument("--filename", default="/home/manbing/Documents/Data/jiahui/MC_Events/BeMC_histmass_vsekin_jiahuicommsel.npz",   help="Path to root file to read tree from")
    parser.add_argument("--filename", default="/home/manbing/Documents/Data/data_BeP8/BeMassHist/BeISS_masshist_EkinP8GBL_Rebin.npz",   help="Path to root file to read tree from")
    parser.add_argument("--plotdir", default="plots/BeP8/fitDataMassGaus", help="Directory to store plots and result files in.")    
    ...
    with np.load(args.filename) as massfile:
        for dec in detectors:
            for iso in fitIso:
                hist2d_mass_energy[dec][iso] = Histogram.from_file(massfile, f"Be_{dec}OptBe7_mass_ciemat")
    ...
    #definations 
    xbinning = hist2d_mass_energy[dec][iso].binnings[0]
    ...
    dict_pars = {dec: {iso: {par: MGraph(binning=xbinning, labels=["Ekin(GeV/n)", f"{par}"]) for par in par_names} for iso in fitIso} for dec in detectors}
    chisquare = {iso: np.zeros(kNucleiNbinRebin)  for iso in fitIso}

    fit_mass_range = dict()
    normdec={'Tof': 500, 'NaF': 100, 'Agl': 500}
    for iso in fitIso:
        mass_ingev = ISOTOPES_MASS[iso]
        isonum = NUCLEI_NUMBER[iso]
        fit_mass_range[iso] = [1/(mass_ingev*1.1), 1/(mass_ingev * 0.92)]   
        print(iso, fit_mass_range[iso])
    for dec in detectors:
        ...
        for iso in fitIso:
            ...
            bin_range = xbinning.get_indices(ANALYSIS_RANGE_EKIN[dec], with_overflow=False)
            print(xbinning)
            print('bin_range:', bin_range)
            print('ANALYSIS_RANGE_EKIN[dec]:', ANALYSIS_RANGE_EKIN[dec])
            for ibin in range(bin_range[0], bin_range[1]+1):
                ...
                plot1d_errorbar_v2(fig, ax1, xvalues[fit_range[0]: fit_range[1]], counts=hist_ibin.values[1:-1][fit_range[0]: fit_range[1]], err=np.sqrt(hist_ibin.values[1:-1][fit_range[0]: fit_range[1]]), label_x="1/mass", label_y="counts",  style=".", color="black", setlogy=0, markersize=20)
                plot1d_errorbar_v2(fig, ax1, xvalues, counts=hist_ibin.values[1:-1], err=np.sqrt(hist_ibin.values[1:-1]), label_x="1/mass", label_y="counts",  style=".", color="black", setlogy=0, markersize=20)
                ...
                pull = (counts - fit_templates[fit_range[0]: fit_range[1]])/np.sqrt(hist_ibin.values[1:-1][fit_range[0]: fit_range[1]])
                ...
                #plot1d_errorbar_v2(fig, ax2, xvalues, counts=(hist_ibin.values[1:-1]-fit_templates)/np.sqrt(hist_ibin.values[1:-1]), err=np.zeros(len(xvalues)), label_x="Charge", label_y=r"(fit-data)/$\mathrm{\sigma_{data}}$",  style=".", color="black", setlogy=0, markersize=20)
        ...
        for dec in detectors:
            for iso in fitIso:
                for par in dict_pars[dec][iso].keys():
                    dict_pars[dec][iso][par].add_to_file(df_graph_gausfit, f'graph_{dec}{iso}_{par}')
                    
        np.savez(os.path.join(args.plotdir, 'graph_gausfit_MCMass_Be.npz'), **df_graph_gausfit)
        ...
        sub_graph_mean = {dec: {} for dec in detectors}
        for dec in detectors:
            ...
            for iso in fitIso:
                sub_graph_mean[dec][iso] = slice_graph_by_value(dict_pars[dec][iso]["mean"], ANALYSIS_RANGE_EKIN[dec])
                print(dec, iso, sub_graph_mean[dec][iso])
                if iso != "Be10":
                    plot_graph(fig, ax1, sub_graph_mean[dec][iso], color=ISOTOPES_COLOR[iso], label=ISO_LABELS[iso], style="EP", xlog=False, ylog=False, scale=None, markersize=22)
                    popt, pcov = curve_fit(poly, np.log(sub_graph_mean[dec][iso].xvalues), sub_graph_mean[dec][iso].yvalues, p0 = [1/ISOTOPES_MASS[iso], 0.0001])
                    ax1.plot(sub_graph_mean[dec][iso].xvalues, poly(np.log(sub_graph_mean[dec][iso].xvalues), *popt), '-', color=ISOTOPES_COLOR[iso])
                    print(iso, " mean_fit:", popt, " _err:", pcov)
                    pars_mean = np.polyfit(np.log(sub_graph_mean[dec][iso].getx()), sub_graph_mean[dec][iso].gety(), 0)
            ...
            ratio_mean_be9be7 = sub_graph_mean[dec]['Be9']/sub_graph_mean[dec]['Be7']
            ratio_mean_be10be7 = sub_graph_mean[dec]['Be10']/sub_graph_mean[dec]['Be7']
            ...
            plot_graph(fig, ax2, sub_graph_mean[dec]['Be9']/sub_graph_mean[dec]['Be7'], color=ISOTOPES_COLOR['Be9'], style="EP", xlog=False, ylog=False, scale=None, markersize=22)
            plot_graph(fig, ax2, sub_graph_mean[dec]['Be10']/sub_graph_mean[dec]['Be7'], color=ISOTOPES_COLOR['Be10'], style="EP", xlog=False, ylog=False, scale=None, markersize=22)
            ...
            savefig_tofile(fig, args.plotdir, f"mean_compare_be9_{dec}", show=True)
        ...
        for dec in detectors:
            ...
            for iso in fitIso:
                if iso != "Be10":
                    sub_graph_sigma[iso] = slice_graph_by_value(dict_pars[dec][iso]["sigma"], ANALYSIS_RANGE_EKIN[dec])
            ...
            equation_text = r'$\mathrm{\sigma_{p} = p_{0} + p_{1} \cdot x + p_{2} \cdot x^{2} + p_{3} \cdot x^{3}}$'
        ...
        ax1.set_xticklabels([])
        ax2.set_xlabel("Ekin/n (GeV/n)")
        savefig_tofile(fig, args.plotdir, f"sigma_compare_be9_{dec}", show=True)
        ...
        ax1.set_xticklabels([])
        ax2.set_xlabel("Ekin/n (GeV/n)")
        savefig_tofile(fig, args.plotdir, f"sigma_compare_be10_{dec}", show=True)
