...
mcpars_initlims = {'Tof': {'norm': (1, 200), 'mean': (-0.07, 0.001), 'sigma': (0.005, 0.015), 'fraccore': (0.7, 0.95), 'sigma_ratio':(1.0, 2.2), 'asy_factor':(0.85, 3.0)},
                   'NaF': {'norm': (0.01, 50), 'mean': (-0.04, 0.001), 'sigma': (0.0001, 0.002), 'fraccore': (0.3, 1.0), 'sigma_ratio':(1.0, 2.0), 'asy_factor':(0.4, 2.0)},
                   'Agl': {'norm': (0.06, 1), 'mean': (-0.005, 0.001), 'sigma': (0.0003, 0.0006), 'fraccore': (0.5, 1.0), 'sigma_ratio':(1.0, 3.0), 'asy_factor':(0.8, 2.0)}}
...
mcpars_initlims_inversebeta = {'Tof': {'norm': (10, 200), 'mean': (0.000, 0.07), 'sigma': (0.009, 0.015), 'fraccore': (0.7, 0.95), 'sigma_ratio':(1.0, 2.2), 'asy_factor':(0.85, 3.0)},
                               ...
                               'Agl': {'norm': (0.06, 1), 'mean': (0.000, 0.005), 'sigma': (0.0003, 0.0006), 'fraccore': (0.5, 1.0), 'sigma_ratio':(1.0, 3.0), 'asy_factor':(0.8, 2.0)}}
...
beta_range = {"Tof": [0.7, 0.99], "NaF": [0.85, 0.999], "Agl": [0.96, 0.997]}
...
def main():
    ...
    parser.add_argument("--filename_mc", default=f"/home/manbing/Documents/Data/data_{nuclei}P8/Hist2D/LiMC_hist_AllHist1308_Rebin.npz",  help="Path to root file to read tree from")
    parser.add_argument("--plotdir", default=f"plots/unfold/{nuclei}Beta", help="Directory to store plots and result files in.")
    parser.add_argument("--isinverse", default=False, help="Directory to store plots and result files in.")
    ...
    for dec in detectors:
        for iso in ISOTOPES[nuclei]:
            if args.isinverse:
                ...
                df_probbeta_pars[dec][iso] = np.load(f'/home/manbing/Documents/lithiumanalysis/scripts/plots/unfold/{nuclei}Beta/{dec}Li6_polypar_betav0.npz')
        ...
        for dec in detectors:
            for iso in isotopes:
                if args.isinverse:
                    ...
                    hist_resolution[dec][iso] = WeightedHistogram.from_file(mc_file, f"hist2d_beta_residual_vsBeta_{dec}{iso}")
    ...
    np.savez(os.path.join(args.plotdir, f'graph_betaresidual_vsbeta2.npz'), **dnpz_allpars)
