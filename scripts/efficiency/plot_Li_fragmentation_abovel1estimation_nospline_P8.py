...
binIndices = Binning(rigiditybinning).get_indices(RIG_XLIM)
...
def get_acceptance_input_hist(events_pass, file_pgen, trigger, isotope, variable):
    ...
    #print(len(arr_pass), hist_events_pass.axes[0].edges())
    ...
    print(isotope, len(arr_pass), len(arr_tot))
...
def main():
    ...
    parser.add_argument("--resultdir", default="plots/Li_bkg_aboveL1", help="Directory to store plots and result files in.")
    parser.add_argument("--datadir", default="/home/manbing/Documents/Data/data_LiP8/presel_flux", help="Directory to store and take data.")    
    ...
    znum = 'z3'
    file_name_iss = "/home/manbing/Documents/Data/data_BeP8/efficiency/anaflux_ISSP8_GBLV2.root"
    with uproot.open(file_name_iss) as file_iss:
        hist_event_be = file_iss[f"h_event_{znum}_2_1"]
        hist_event_clean_be = file_iss[f"h_event_clean4_{znum}_2_1"]
        print('hist_event_be:', hist_event_be.axes[0].edges())
    #file_names = {"Be7": "/home/manbing/Documents/Data/data_BeP8/efficiency/Be7MC_B1236_anaflux_GBL.root",
    #              "Be9": "/home/manbing/Documents/Data/data_BeP8/efficiency/Be9MC_B1236_anaflux_GBL.root",
    #              "Be10": "/home/manbing/Documents/Data/data_BeP8/efficiency/Be10MC_B1236_anaflux_GBL.root",
    #              "B10": "/home/manbing/Documents/Data/data_BeP8/efficiency/B10MC_B1236_anaflux_GBL.root",
    ...
    fragment_nuclei = {"Be7": 'Z4', 'Be9': 'Z4', 'Be10': 'Z4', "B10": "Z5", "B11": "Z5","C12": "Z6", "N14": "Z7", "N15": "Z7", "O16": "Z8"}
    #fragment_nuclei = {"Be7": "Z4"}
    file_names = {iso: f'/home/manbing/Documents/Data/data_BeP8/efficiency/{iso}MC_B1236_anaflux_GBLV2.root' for iso in fragment_nuclei.keys()}
    ...
    file_flux_qy = {"C12": '/home/manbing/Documents/Data/data_flux_qy/carbon64_20200420V2N_B1215401R4MCKY10COMBUNFOLDTOINB_totalQYAN.root',
                    ...
                    'Be7': '/home/manbing/Documents/Data/data_flux_qy/beryllium64_20200420V2N_B1219401R4MCKY10COMBUNFOLDTOINB_totalQYAN.root',
                    'Be9': '/home/manbing/Documents/Data/data_flux_qy/beryllium64_20200420V2N_B1219401R4MCKY10COMBUNFOLDTOINB_totalQYAN.root',
                    'Be10': '/home/manbing/Documents/Data/data_flux_qy/beryllium64_20200420V2N_B1219401R4MCKY10COMBUNFOLDTOINB_totalQYAN.root'}
    ...
    for iso, z_name in fragment_nuclei.items():
        ...
        with uproot.open(file_flux_qy[iso]) as file_ref_flux:
            ...
            print(iso, 'hist_ref_flux[iso]:', hist_ref_flux[iso].axes[0].edges()) 
        
        with uproot.open(file_names[iso]) as rootfile:
            hist_event[iso] =  rootfile[f"h_event_gen_{znum}_2"]
            hist_event_nofragl1[iso] =  rootfile[f"h_event_gen_l1_{znum}_2"]
            
            hist_event_clean[iso] =  rootfile[f"h_event_gen_clean_{znum}_2"] 
            hist_event_clean_nofragl1[iso] =  rootfile[f"h_event_gen_clean_l1_{znum}_2"] 
            print(iso, hist_event_clean[iso].axes[0].edges())
    ...
    nuclei = ['Be', "B", "C", "N", "O"]
    nuclei_label = ['Be->Li', "B->Li", "C->Li", "N->Li", "O->Li"]
    iso_ratio_guess = {"Be7": 0.6, 'Be9': 0.3, 'Be10': 0.1, "B10": 0.5, "B11":0.5, "C12":1.0, "O16": 1.0, "N14": 0.5, "N15": 0.5}
    COLOR_NUCLEI = {"Be": "tab:orange", "B": "tab:blue", "C": "tab:green", "N":"magenta", "O": "tab:red"}
    ...
    for i_nuclei in nuclei:
        ...
        ax1.plot(rig_binning.bin_centers[1:-1], yfit_frag_ratio, "-", color=COLOR_NUCLEI[i_nuclei], label=f'{i_nuclei}->Li')
    ...
    for i, i_nuclei in enumerate(nuclei):
        ...
        ax1.plot(rig_binning.bin_centers[1:-1], yfit_bkg_stackup[i+1, :], "-", color=COLOR_NUCLEI[i_nuclei], linewidth=3, label=f'{i_nuclei}->Li')
    ...
    ax1.set_ylabel(r"$\mathrm{N_{x, Li}/N_{Li^{'}}}$", fontsize=FONTSIZE+2)
    ax1.set_xlabel(r"$\mathrm{Rigidity (GV)}$", fontsize=FONTSIZE+2)
    ax1.set_xscale('log')
    setplot_defaultstyle()
    set_plot_defaultstyle(ax1)
    ax1.set_xlim([1.8, 1300])
    ...
    ax1.legend(handles, nuclei_label, fontsize=FONTSIZE+2)
    ...
    for i_nuclei in nuclei:
        ...
        ax1.plot(rig_binning.bin_centers[1:-1], yfit_frag_ratio_clean, "-", color=COLOR_NUCLEI[i_nuclei], label=f'{i_nuclei}->Li')
    ...
    for i, i_nuclei in enumerate(nuclei):
        ...
        ax1.plot(rig_binning.bin_centers[1:-1], yfit_bkg_stackup_clean[i, :], "-", color=COLOR_NUCLEI[i_nuclei], linewidth=3, label=f'{i_nuclei}->Li')

    
    ax1.set_ylabel(r"$\mathrm{N_{x, Li}/N_{Li^{'}}}$", fontsize=FONTSIZE)
    ax1.set_xlabel(r"$\mathrm{Rigidity (GV)}$", fontsize=FONTSIZE)
    ax1.set_xscale('log')
    setplot_defaultstyle()
    set_plot_defaultstyle(ax1)
    handles = [mpatches.Patch(color=c) for c in COLOR_NUCLEI.values()]
    ax1.legend(handles, nuclei_label, fontsize=FONTSIZE)
    ax1.set_xlim([1.8, 1300])
    ax1.set_ylim([0.0, 0.1])
    ax1.set_xticks([2, 5, 10, 30,  100,  300, 1000])
