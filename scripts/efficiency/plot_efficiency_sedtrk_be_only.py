...
from tools.constants import NUCLEI_CHARGE, ISO_LABELS
...
effcorplotlim = {"background": {"He": [0.98, 1.01], "C": [0.94, 1.0], "Be": [0.9, 1.0]}}
...
def get_efficiency(nuclei, filename, datatype, variable):
    ...
    with uproot.open(filename) as rootfile:
        for i, cut in enumerate(studyeff):
            ...
            savefig_tofile(figure, "plots/sectrk", f"hist_{nuclei}{datatype}_{cut}_passevent", show=False)
...
def main():
    ...
    #parser.add_argument("--filename_iss", default="/home/manbing/Documents/Data/data_effacc/annflux/anaflux_ISSP8_GBL.root", help="Path to root file to read tree from")
    ...
    with np.load(os.path.join(args.datadir, 'graph_issbkgeff_factor.npz')) as file_bkg:
        graph_issbkgeff_factor = MGraph.from_file(file_bkg, "graph_issbkgeff_factor")
        print(len(graph_issbkgeff_factor.xvalues), graph_issbkgeff_factor.xvalues)
        print(len(graph_issbkgeff_factor.yvalues), graph_issbkgeff_factor.yvalues)
        
        spline_graph_issbkgeff_factor = UnivariateSpline(np.log(graph_issbkgeff_factor.xvalues), graph_issbkgeff_factor.yvalues, k=3, s=10)

    eff_iss_signal = dict()
    dictgraph_eff_iss_signal = dict()
    for nuclei in nucleus:
        ...
        for cut in studyeff:  
            ...
            factor_isseff = spline_graph_issbkgeff_factor(np.log(dictgraph_eff_iss[nuclei][cut].xvalues))
            print(dictgraph_eff_iss[nuclei][cut].xvalues)
            print(len(factor_isseff), factor_isseff)
            eff_iss_signal[nuclei] = dictgraph_eff_iss[nuclei][cut].yvalues/graph_issbkgeff_factor.yvalues
            dictgraph_eff_iss_signal[nuclei] = MGraph(dictgraph_eff_iss[nuclei][cut].xvalues, eff_iss_signal[nuclei],  dictgraph_eff_iss[nuclei][cut].yerrs/factor_isseff)
            #plot_graph_eff(figure, ax1, dictgraph_eff_iss_signal[nuclei], "ISS", col="red", legend=f"{nuclei} ISS Corrected")

            for i, isotope in enumerate(isotopes[nuclei]):
                plot_graph_eff(figure, ax1, dictgraph_eff_mc[nuclei][isotope][cut], isotope, legend=f"MC {ISO_LABELS[isotope]}")
            ...
            plt.grid()
            ax1.get_yticklabels()[0].set_visible(False)
            savefig_tofile(figure, args.resultdir, f"hist_{nuclei}_{cut}_rig_efficiency_v2", show=True)

    for nuclei in nucleus:
        figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(20, 15))
        dictgraph_eff_iss_signal[nuclei] = MGraph(dictgraph_eff_iss[nuclei][cut].xvalues, eff_iss_signal[nuclei],  dictgraph_eff_iss[nuclei][cut].yerrs/factor_isseff)            
        plot_graph_eff(figure, ax1, dictgraph_eff_iss_signal[nuclei], "ISS", col="red", legend=f"ISS {nuclei} Corrected")
        for cut in studyeff:
            for i, isotope in enumerate(isotopes[nuclei]):
                plot_graph_eff(figure, ax1, dictgraph_eff_mc[nuclei][isotope][cut], isotope, legend=f"MC {ISO_LABELS[isotope]}")
                #ax2.plot(dictgraph_eff_iss_signal[nuclei].xvalues, dictgraph_eff_iss_signal[nuclei].yvalues/dictgraph_eff_mc[nuclei][isotope][cut].yvalues, ".", color=color_iso[isotope], linewidth=3, label=f"MC {isotope}")
                graph_effcor_bkgcut = dictgraph_eff_iss_signal[nuclei]/dictgraph_eff_mc[nuclei][isotope][cut]
                plot1d_errorbar_v2(figure, ax2,  dictgraph_eff_iss_signal[nuclei].getx(), dictgraph_eff_iss_signal[nuclei].yvalues/dictgraph_eff_mc[nuclei][isotope][cut].yvalues, err=graph_effcor_bkgcut.yerrs, label_x="Rigidity (GV)", label_y="Efficiency", color=color_iso[isotope], style=".", legendfontsize=FONTSIZE, setlogx=True, setlogy=False, setscilabelx=False, setscilabely=False, markersize=25)
                #plot_curvefit(figure, ax2, dict_eff_cor[nuclei][isotope][cut], poly_func, dict_polyfitpars_effcor[nuclei][isotope][cut])
            ...
            plt.grid()
            ax1.get_yticklabels()[0].set_visible(False)
            savefig_tofile(figure, args.resultdir, f"hist_{nuclei}_{cut}_rig_efficiency_corrected", show=True)
    ...
    ax1.plot(graph_issbkgeff_factor.xvalues, graph_issbkgeff_factor.yvalues, "-", color="black", linewidth=3, label=r"$\mathrm{\alpha}")
    ax1.legend(fontsize=FONTSIZE)
    ax1.set_ylabel(r"$\mathrm{\alpha}", fontsize=FONTSIZE)
