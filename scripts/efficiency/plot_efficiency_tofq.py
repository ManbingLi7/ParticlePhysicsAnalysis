...
cutplotlim = {"l1q": [0.95, 1.02], "tof": [0.96, 1.005], "inntof": [0.78, 0.95], "inncutoff": [0.78, 0.95], "pk": [0.7, 1.0], "bz" : [0.65, 0.85], "trigger": [0.985, 1.005]}
...
def main():
    ...
    parser.add_argument("--resultdir", default="plots/tofq", help="Directory to store plots and result files in.")
    parser.add_argument("--variable", default="Rigidity", help="dataname for the output file and plots.")
    parser.add_argument("--savedatadir", default="plots/tofq", help="dataname for the output file and plots.")    
    ...
    pd_jiahui_tofq = pd.read_csv("/home/manbing/Documents/Data/jiahui/efficiencies/utofq_eff_c.csv",  sep='\s+', header=0)
    print(pd_jiahui_tofq)
    graph_tofq_jiahui = MGraph(xvalues=pd_jiahui_tofq["bin_center[GV]"], yvalues=pd_jiahui_tofq["dt"]/pd_jiahui_tofq["C"],
                                  yerrs=get_ratio_err(pd_jiahui_tofq["dt"], pd_jiahui_tofq["C"], pd_jiahui_tofq["dt_err"], pd_jiahui_tofq["C_err"]))

    graph_tofq_jiahui_iss = MGraph(xvalues=pd_jiahui_tofq["bin_center[GV]"], yvalues=pd_jiahui_tofq["dt"], yerrs=pd_jiahui_tofq["dt_err"])
    graph_tofq_jiahui_mc = MGraph(xvalues=pd_jiahui_tofq["bin_center[GV]"], yvalues=pd_jiahui_tofq["C"], yerrs=pd_jiahui_tofq["C_err"])
    ...
    for nuclei in nucleus:
        ...
        for cut in studyeff:  
            ...
            for i, isotope in enumerate(isotopes[nuclei]):
                ...
                if "inn" not in cut:
                    plot_curvefit(figure, ax2, dict_eff_cor[nuclei][isotope][cut], poly_func, dict_polyfitpars_effcor[nuclei][isotope][cut], col="tab:orange", label="this")

            plot1d_errorbar_v2(figure, ax1,  graph_tofq_jiahui_iss.getx(), graph_tofq_jiahui_iss.gety(), err=graph_tofq_jiahui_iss.get_yerrors(), label="Jiahui ISS", color="grey", style=".", markersize=20)
            plot1d_errorbar_v2(figure, ax1,  graph_tofq_jiahui_mc.getx(), graph_tofq_jiahui_mc.gety(), err=graph_tofq_jiahui_mc.get_yerrors(), label="Jiahui MC", color="magenta", style=".", markersize=20)
            plot1d_errorbar_v2(figure, ax2,  graph_tofq_jiahui.getx(), graph_tofq_jiahui.gety(), err=graph_tofq_jiahui.get_yerrors(), label="Jiahui", color="grey", style=".", markersize=20)
            ...
            ax1.text(0.85, 0.95, f'{nuclei}', fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="tab:orange", fontweight="bold")
    ...
    graph_utof_q = dict_eff_cor[nuclei][isotopes[nuclei][0]]["tof"]
    plot_graph_eff(figure, ax1, graph_utof_q, "C12", legend="this", ylabel="ISS/MC(C12)")
    fit_utof_q_eff = poly_func(np.log(kNucleiBinsRebin_center), *dict_polyfitpars_effcor[nuclei]["C12"]["tof"])
    #fit_bz_eff[graph_utof_q.get_index(108):] = fit_bz_eff[graph_tofq.get_index(108)] * np.ones_like(fit_bz_eff[graph_utof_q.get_index(108):])
    
    ax1.plot(graph_utof_q.xvalues, eff_cor_utof_q_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")
    #ax1.plot(graph_utof_q.xvalues, fit_utof_q_eff, "-", color="tab:orange")
    print(graph_utof_q.xvalues, len(graph_utof_q.xvalues))
    print(graph_tofq_jiahui.getx(), len(graph_utof_q.xvalues))
    
    plot1d_errorbar_v2(figure, ax1,  graph_tofq_jiahui.getx(), graph_tofq_jiahui.gety(), err=graph_tofq_jiahui.get_yerrors(), label="Jiahui", color="grey", style=".", markersize=20)
    #ax2.plot(graph_utof_q.xvalues, fit_utof_q_eff/eff_cor_utof_q_jw(np.log(kNucleiBinsRebin_center)), "-", color="orange", label="ratio: fit")
    ax2.plot(graph_utof_q.xvalues, graph_utof_q.yvalues/graph_tofq_jiahui.gety(), ".", markersize=20, color="black", label="ratio: data")

    spline_tofq_effcor = UnivariateSpline(np.log(graph_utof_q.xvalues[5:-1]), graph_utof_q.yvalues[5:-1], k=3, s=100)
    tofq_eff_spline = spline_tofq_effcor(np.log(graph_utof_q.xvalues))
    ax1.plot(graph_utof_q.xvalues, tofq_eff_spline, "-", markersize=20, color="tab:orange")
    ax2.plot(graph_utof_q.xvalues, tofq_eff_spline/eff_cor_utof_q_jw(np.log(kNucleiBinsRebin_center)), "-", color="red", label="ratio: fit")
    ax2.grid(linewidth=1)
    ax1.set_ylim([0.985, 1.005])
    ax2.set_ylim([0.9985, 1.001])
    ax1.set_xscale("log")
    ax2.set_xscale("log")
    ...
    ax1.text(0.85, 0.95, f'{nuclei}', fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="tab:orange", fontweight="bold")
    figure.subplots_adjust(left= 0.15, right=0.96, bottom=0.1, top=0.95)

    plt.subplots_adjust(hspace=.0)
    set_plot_defaultstyle(ax2)   
    savefig_tofile(figure, args.resultdir, f"hist_compare_jw_utofq_effcor", show=True)

    dict_graph_tofq = dict()
    graph_utof_q.add_to_file(dict_graph_tofq, "graph_tofq_effcor")
    np.savez(os.path.join(args.savedatadir, f"graph_tofq_effcor.npz"), **dict_graph_tofq)
    np.savez(os.path.join(args.savedatadir, f"graph_tofq_eff.npz"), **df_graph_effissmc)  
    save_spline_to_file(spline_tofq_effcor, args.savedatadir, "spline_tofq_effcor.pickle")
