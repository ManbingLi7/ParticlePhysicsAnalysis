...
def main():
    ...
    parser.add_argument("--filename_iss", default="/home/manbing/Documents/lithiumanalysis/Scripts/trees/ISS/BeISS_BetaCor.root", help="Path to root file to read tree from")
    parser.add_argument("--filenames_mc", default=["trees/MC/Be7MC_BetaCor.root", 
                                                   "trees/MC/Be10MC_BetaCor.root",
                                                   "trees/MC/Be10MC_BetaCor.root"], help="Path to root file to read tree from")
    ...
    parser.add_argument("--dataname", default="NaFISS", help="dataname for the output file and plots.")
    parser.add_argument("--detector", default="NaF", help="dataname for the output file and plots.")
    ...
    with uproot3.open(args.filename_iss) as isstreefile:
         isstree = isstreefile["amstreea_cor"]                                                                                                                                               
    ...
    for i, isotope in enumerate(isotopes[nuclei]):
        ...
        for events in read_tree(filename_mc, "amstreea_cor", chunk_size=args.chunk_size):
             eventsmctot = selector_naf_event(events)
             hist_ekin_mctot = fill_hist_ekin_counts(eventsmctot, ekinbinning, isdata=False)
             
             eventsmcpass = selector_naf_ciematvar(events, nuclei, isotope, isdata=False) 
             hist_ekin_mcpass = fill_hist_ekin_counts(eventsmcpass, ekinbinning, isdata=False)  


        effnaf_mcbe, efferrnaf_mcbe = calculate_efficiency_and_error(hist_ekin_mcpass.values, hist_ekin_mctot.values)
        figure = plt.figure(figsize=FIGSIZE_BIG)                                                                                                                                                        
        plot = figure.subplots(1, 1)    
        ...
        plot.legend()
        eff_cor = effnaf_beiss/effnaf_mcbe
        dict_eff_cor[isotope] = eff_cor
        #hist_effcor = Histogram(ekinbinning, values=eff_cor, labels=["Ekin/n (GeV/n)", "Eff Correction"])
        np.savez(os.path.join(args.resultdir, f"{isotope}eff_correction_naf.npz"), xbincenter=ekinbinning.bin_centers,  eff_cor=eff_cor)
        print(eff_cor)                               
        figure = plt.figure(figsize=FIGSIZE_BIG)                                                                                         
        plot = figure.subplots(1, 1)                                                                                                                                                                       
        plot1dhist(figure, plot, hist_ekin_pass.binnings[0].edges, effnaf_beiss,  efferrnaf_beiss, "Ekin/n (GeV/n)", "Efficiency", "ISS",  "black", 30, 1, 0, 0, 1)
        plot1dhist(figure, plot, hist_ekin_mcpass.binnings[0].edges, effnaf_mcbe,  efferrnaf_mcbe, "Ekin/n (GeV/n)", "Efficiency", f"{isotope}MC",  "blue", 30, 1, 0, 0, 1)                                
        figure.savefig("trees/acceptance/hist_{}_ekin_efficiency.pdf".format(args.dataname), dpi=250)    

    color = {"Be7": "tab:orange", "Be9": "tab:blue", "Be10": 'tab:red'}
    figure = plt.figure(figsize=FIGSIZE_BIG)                                                                                         
    plot = figure.subplots(1, 1)
    for isotope in isotopes[nuclei]:
        plot1dhist(figure, plot, hist_ekin_mcpass.binnings[0].edges, dict_eff_cor[isotope],  np.zeros(len(dict_eff_cor[isotope])), "Ekin/n (GeV/n)", "Eff Corr", f"{isotope}",  color[isotope], 30, 1, 0, 0, 1)    
    figure.savefig("trees/acceptance/hist_{}_ekin_efficiencycorrection.pdf".format(args.dataname), dpi=250)    
