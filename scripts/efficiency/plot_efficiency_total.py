...
def plot_comparison_twohist(figure=None, ax1=None, ax2=None, histA=None, histB=None, ratio=None, ratioerr=None, xlog=True, ylog=False, isotope=None, xlabel=None, ylabel=None, figname=None, save=False, cutname=None):
    ...
    ax2.sharex(ax1)
    if (save):
        savefig_tofile(figure, "plots", f"hist_{figname}_rig_passtotalevent", show=False)
...
def main():
    ...
    parser.add_argument("--datadir", default="/home/manbing/Documents/Data/data_effacc/eff_corrections", help="dataname for the output file and plots.")
    ...
    spline_effcor_bz_jw = data_jiahui_eff[2][0](np.log(kNucleiBinsRebin_center))  
    spline_effcor_trigger_jw =  data_jiahui_eff[2][1](np.log(kNucleiBinsRebin_center))
    spline_effcor_utofq_jw =  data_jiahui_eff[2][2](np.log(kNucleiBinsRebin_center))
    spline_effcor_l1q_jw = data_jiahui_eff[2][3](np.log(kNucleiBinsRebin_center))
    spline_effcor_inntrk_jw = data_jiahui_eff[2][4](np.log(kNucleiBinsRebin_center))
    spline_effcor_tofbeta_jw = data_jiahui_eff[4][0](np.log(kNucleiBinsRebin_center))
    spline_effcor_naf_jw = data_jiahui_eff[4][1](np.log(kNucleiBinsRebin_center))
    spline_effcor_agl_jw = data_jiahui_eff[4][2](np.log(kNucleiBinsRebin_center))
    ...
    spline_bz_effcor = get_spline_from_file(os.path.join(args.datadir, "spline_bz_eff.pickle"), "spline_bz_eff")
    ...
    spline_tofvelocity_effcor = get_spline_from_file(os.path.join(args.datadir, "spline_tofvelocity_effcor.pickle"), "spline_tofvelocity_effcor")
    tofvelocity_effcor = spline_tofvelocity_effcor(np.log(kNucleiBinsRebin_center))
    xbinning = Binning(kNucleiBinsRebin)
    tran_index = xbinning.get_indices([80])[0]
    tofvelocity_effcor[tran_index:] = tofvelocity_effcor[tran_index] * np.ones_like(tofvelocity_effcor[tran_index:])

    spline_naf_effcor = get_spline_from_file(os.path.join(args.datadir, "spline_NaF_effcor_v1.pickle"), "spline_rich_effcor_new_NaF")
    naf_effcor = spline_naf_effcor(np.log(kNucleiBinsRebin_center))

    spline_agl_effcor = get_spline_from_file(os.path.join(args.datadir, "spline_Agl_effcor.pickle"), "spline_Agl_effcor")
    agl_effcor = spline_agl_effcor(np.log(kNucleiBinsRebin_center))

    spline_innq_effcor = get_spline_from_file(os.path.join(args.datadir, "spline_innq_effcor.pickle"), "spline_innq_effcor")
    innq_effcor = spline_agl_effcor(np.log(kNucleiBinsRebin_center))

    print(tofvelocity_effcor)
    with np.load(os.path.join(args.datadir,  "graph_bz_effcor.npz")) as file_bz:
        graph_be_effcor = MGraph.from_file(file_bz, "graph_bz_effcor")
    ...
    #Tof
    figure, ax1 = plt.subplots(1, 1, figsize=(18, 9))
    figure.subplots_adjust(left= 0.13, right=0.96, bottom=0.1, top=0.95)
    ax1.plot(kNucleiBinsRebin_center, bz_effcor, "-", color="tab:orange", label="Big-Z")
    ax1.plot(kNucleiBinsRebin_center, tofq_effcor, "-", color="tab:green", label="UTof Q")
    ax1.plot(kNucleiBinsRebin_center, trigger_effcor, "-", color="magenta", label="Trigger")
    ax1.plot(kNucleiBinsRebin_center, l1q_effcor, "-", color="cyan", label="L1Q")
    ax1.plot(kNucleiBinsRebin_center, tofvelocity_effcor, "-", color="brown", label="Tof-V")
    ax1.plot(graph_inntrk_effcor.xvalues, graph_inntrk_effcor.yvalues, "-", color="tab:blue", label="Inn Trk")
    ax1.plot(kNucleiBinsRebin_center, innq_effcor, "-", color="grey", label="Inn Trk Q")


    print(kNucleiBinsRebin_center)
    print(graph_inntrk_effcor.xvalues)

    total_effcor_tof = bz_effcor[5:] * tofq_effcor[5:] * trigger_effcor[5:] * l1q_effcor[5:] * tofvelocity_effcor[5:] * graph_inntrk_effcor.yvalues * innq_effcor[5:]
    ...
    spline_total_effcor_tof =  UnivariateSpline(np.log(graph_total_effcor_tof.getx()[1:20]), total_effcor_tof[1:20], k=3, s=5)   
    ax1.plot(kNucleiBinsRebin_center[5:], spline_total_effcor_tof(np.log(graph_total_effcor_tof.getx())), '--', label='Total', color='black', linewidth=3)

    #ax1.plot(kNucleiBinsRebin_center[5:], total_effcor_tof, "-", color="black", linewidth=3, label="Total") 
    #plot_graph(figure, ax1, graph_be_effcor,  color="tab:orange", label=f"Jiahui ISS", style="EP", xlog=True, markersize=20)
    #ax1.plot(kNucleiBinsRebin_center[5:], effcor_total_jw_tof[5:], "--", color="red", linewidth=3, label="Jiahui") 
    ...
    ax1.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ax1.get_yticklabels()[0].set_visible(False)
    ax1.set_xlabel(RIG_XLABEL)
    ax1.set_ylabel("Efficiency Corrections")
    set_plot_defaultstyle(ax1)
    legend = plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.subplots_adjust(right=0.75) 
    ax1.fill_betweenx(np.linspace(0.9, 1.05, 100), ANALYSIS_RANGE_RIG[nuclei]["Tof"][0], 5.0, alpha=0.1, color='tab:blue')
    ...
    savefig_tofile(figure, args.resultdir, f"hist_total_effcor_tof", show=True)
    ...
    ax1.plot(kNucleiBinsRebin_center, tofq_effcor, "-", color="tab:green", label="UTof Q")
    ax1.plot(kNucleiBinsRebin_center, trigger_effcor, "-", color="magenta", label="Trigger")
    ax1.plot(kNucleiBinsRebin_center, l1q_effcor, "-", color="red", label="L1Q")
    ax1.plot(kNucleiBinsRebin_center, naf_effcor, "-", color="brown", label="RICH NaF")
    ax1.plot(graph_inntrk_effcor.xvalues, graph_inntrk_effcor.yvalues, "-", color="tab:blue", label="Inn Trk")
    ax1.plot(kNucleiBinsRebin_center, innq_effcor, "-", color="grey", label="Inn Trk Q")
    print(kNucleiBinsRebin_center)
    print(graph_inntrk_effcor.xvalues)

    total_effcor_naf = bz_effcor[5:] * tofq_effcor[5:] * trigger_effcor[5:] * l1q_effcor[5:] * graph_inntrk_effcor.yvalues *naf_effcor[5:] * innq_effcor[5:]

    graph_total_effcor_naf = MGraph(kNucleiBinsRebin_center[5:], total_effcor_naf, yerrs=np.zeros_like(total_effcor_naf))
    
    graph_total_effcor_naf.add_to_file(dict_total_effcor, "graph_total_effcor_naf")
    spline_total_effcor_naf =  UnivariateSpline(np.log(graph_total_effcor_naf.getx()), total_effcor_naf, k=3, s=5)   
    ax1.plot(kNucleiBinsRebin_center[5:], spline_total_effcor_naf(np.log(graph_total_effcor_naf.getx())), '--', label='Total', linewidth=3, color='black')
    #ax1.plot(kNucleiBinsRebin_center[5:], total_effcor_naf, "-", color="black", linewidth=3, label="Total")
    #ax1.plot(kNucleiBinsRebin_center[5:], effcor_total_jw_naf[5:], "--", color="red", linewidth=3, label="Jiahui") 
    ax1.grid(linewidth=1)
    ax1.legend(fontsize=FONTSIZE, loc="lower right")
    ax1.set_ylim([0.82, 1.05])
    ax1.set_xscale("log")
    ax1.set_xlim([2, 40])
    ax1.set_xticks([3, 5,  10, 30,  40])
    ax1.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ax1.get_yticklabels()[0].set_visible(False)
    ax1.set_xlabel(RIG_XLABEL)
    ax1.set_ylabel("Efficiency Corrections")
    ...
    legend = plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.subplots_adjust(right=0.75) 
    ax1.fill_betweenx(np.linspace(0.82, 1.05, 100), ANALYSIS_RANGE_RIG[nuclei]["NaF"][0], ANALYSIS_RANGE_RIG[nuclei]["NaF"][1], alpha=0.1, color='tab:blue')

    ax1.text(0.1, 0.98, f"RICH NaF", fontsize=FONTSIZE_BIG, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    savefig_tofile(figure, args.resultdir, f"hist_total_effcor_naf", show=True)
    ...
    ax1.plot(kNucleiBinsRebin_center, trigger_effcor, "-", color="magenta", label="Trigger")
    ax1.plot(kNucleiBinsRebin_center, l1q_effcor, "-", color="red", label="L1Q")
    ax1.plot(kNucleiBinsRebin_center, agl_effcor, "-", color="brown", label="RICH Agl")
    ax1.plot(graph_inntrk_effcor.xvalues, graph_inntrk_effcor.yvalues, "-", color="tab:blue", label="Inn Trk")
    ax1.plot(kNucleiBinsRebin_center, innq_effcor, "-", color="grey", label="Inn Trk Q")
    print(kNucleiBinsRebin_center)
    print(graph_inntrk_effcor.xvalues)

    total_effcor_agl = bz_effcor[5:] * tofq_effcor[5:] * trigger_effcor[5:] * l1q_effcor[5:] * graph_inntrk_effcor.yvalues * agl_effcor[5:] *innq_effcor[5:]  
    graph_total_effcor_agl = MGraph(kNucleiBinsRebin_center[5:], total_effcor_agl, yerrs=np.zeros_like(total_effcor_agl))
    graph_total_effcor_agl.add_to_file(dict_total_effcor, "graph_total_effcor_agl")

    spline_total_effcor_agl =  UnivariateSpline(np.log(graph_total_effcor_agl.getx()), total_effcor_agl, k=3, s=5)   
    ax1.plot(kNucleiBinsRebin_center[5:], spline_total_effcor_agl(np.log(graph_total_effcor_agl.getx())), '--', label='Total', linewidth=3, color='black')
    #ax1.plot(kNucleiBinsRebin_center[5:], total_effcor_agl, "-", color="black", linewidth=3, label="Total")
    #ax1.plot(kNucleiBinsRebin_center[5:], effcor_total_jw_agl[5:], "--", color="red", linewidth=3, label="Jiahui") 
    #plot_graph(figure, ax1, graph_be_effcor,  color="tab:orange", label=f"Jiahui ISS", style="EP", xlog=True, markersize=20)
    ...
    legend = plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.subplots_adjust(right=0.75) 
    ax1.fill_betweenx(np.linspace(0.85, 1.05, 100), ANALYSIS_RANGE_RIG[nuclei]["Agl"][0], ANALYSIS_RANGE_RIG[nuclei]["Agl"][1], alpha=0.1, color='tab:blue')
    ax1.text(0.1, 0.98, f"RICH Aerogel", fontsize=FONTSIZE_BIG, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    savefig_tofile(figure, args.resultdir, f"hist_total_effcor_agl", show=True)

    plt.show()

    np.savez(os.path.join(args.datadir, f"graph_total_effcor.npz"), **dict_total_effcor)  
