...
def main():
    ...
    spline_dict = {iso: dict() for iso in isotopes[nuclei]}
    ...
    spline_naf_effcor = get_spline_from_file(os.path.join(args.datadir, "spline_NaF_effcor.pickle"), "spline_NaF_effcor")
    ...
    #To
    figure, ax1 = plt.subplots(1, 1, figsize=(18, 9))
    figure.subplots_adjust(left= 0.13, right=0.96, bottom=0.13, top=0.95)
    set_plot_style(ax1)
    ...
    ax1.plot(kNucleiBinsRebin_center, innq_effcor, "-", color="red", label="Inn Trk Q")    
    ...
    #ax1.set_xscale("log")
    ax1.set_xlim([ANALYSIS_RANGE_RIG[nuclei]['Tof'][0], 10])
    ...
    legend = plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=30)
    ...
    savefig_tofile(figure, args.resultdir, f"{nuclei}hist_total_effcor_tof", show=True)
    ...
    #ax1.set_xscale()
    #ax1.set_xlim([ANALYSIS_RANGE_RIG[nuclei]['NaF'][0], 20])
    ax1.set_xlim([4.0, 20])
    ...
    savefig_tofile(figure, args.resultdir, f"{nuclei}hist_total_effcor_naf", show=True)
    ...
    #ax1.set_xscale("log")
    #ax1.set_xlim([ANALYSIS_RANGE_RIG[nuclei]['Agl'][0], 40])
    ax1.set_xlim([9.7, 40])
    ...
    savefig_tofile(figure, args.resultdir, f"{nuclei}hist_total_effcor_agl", show=True)

    plt.show()

    np.savez(os.path.join(args.datadir, f"{nuclei}graph_total_effcor.npz"), **dict_total_effcor)  
