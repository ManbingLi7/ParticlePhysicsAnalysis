...
cutname = {"l1q": "L1 unbiased charge", "tof": "UpTof Q", "inntof": "Inner Track(Rtof)", "inncutoff": "Inner Track(Rcutoff)", "pk": "L1 picking up", "bz" : "L1 big charge", "trigger": "Trigger"}
cutplotlim = {"l1q": [0.95, 1.02], "tof": [0.975, 1.005], "inntof": [0.78, 0.95], "inncutoff": [0.78, 0.95], "pk": [0.7, 1.0], "bz" : [0.65, 0.85], "trigger": [0.985, 1.005]}
effcorplotlim = {"l1q": [0.95, 1.05], "tof": [0.97, 1.02], "inntof": [0.9, 1.08], "inncutoff": [0.9, 1.08], "pk": [0.9, 1.1], "bz" : [0.95, 1.075], "trigger": [0.985, 1.015]}
...
def main():
    ...
    for nuclei in nucleus:
        ...
        figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
        transit_binindex = dictgraph_eff_iss[nuclei]["inntof"].get_index(14)
        fit_index_min = dictgraph_eff_iss[nuclei]["inntof"].get_index(1.9)
        fit_index_max = dictgraph_eff_iss[nuclei]["inntof"].get_index(20)
        graph_inntrack_eff_iss[nuclei] = concatenate_graphs(slice_graph(dictgraph_eff_iss[nuclei]["inntof"], 1, 15), slice_graph(dictgraph_eff_iss[nuclei]["inncutoff"], 15, -1))
        show_graph_inntrack_eff_iss[nuclei] = concatenate_graphs(slice_graph(dictgraph_eff_iss[nuclei]["inntof"], 1, 15), slice_graph(dictgraph_eff_iss[nuclei]["inncutoff"], 15, fit_index_max-2))
        plot_graph_eff(figure, ax1, show_graph_inntrack_eff_iss[nuclei], "ISS", legend="ISS")
        for i, isotope in enumerate(isotopes[nuclei]):
            ...
            graph_inntrack_effcor[nuclei][isotope] = concatenate_graphs(slice_graph(dict_eff_cor[nuclei][isotope]["inntof"] , 1, 15), slice_graph(dict_eff_cor[nuclei][isotope]["inncutoff"], 15, -1))
            #y_inntrack_effcor = graph_inntrack_effcor[nuclei][isotope].yvalues
            #y_inntrack_effcor[27:] = y_inntrack_effcor[27]
            #graph_inntrack_effcor[nuclei][isotope].yvalues = y_inntrack_effcor
            #graph_inntrack_effcor[nuclei][isotope].yerrs[27:] =  graph_inntrack_effcor[nuclei][isotope].yerrs[27]
            graph_inntrack_effcor[nuclei][isotope].yvalues[transit_binindex:] = graph_inntrack_effcor[nuclei][isotope].yvalues[transit_binindex-1] * np.ones_like(graph_inntrack_effcor[nuclei][isotope].yvalues[transit_binindex:])
            graph_inntrack_effcor[nuclei][isotope].yerrs[transit_binindex:] = graph_inntrack_effcor[nuclei][isotope].yerrs[transit_binindex-1] * np.ones_like(graph_inntrack_effcor[nuclei][isotope].yerrs[transit_binindex:])
            sliced_graph_inntrack_effcor = slice_graph(graph_inntrack_effcor[nuclei][isotope], fit_index_min, fit_index_max)
            plot_graph_eff(figure, ax1, graph_inntrack_eff_mc[nuclei][isotope], isotope, legend=f"{isotope}")
            plot_graph_eff(figure, ax2, sliced_graph_inntrack_effcor, isotope)
            fit_coeffs, _ = curve_fit(poly_func, np.log(graph_inntrack_effcor[nuclei][isotope].getx()[fit_index_min:fit_index_max]), graph_inntrack_effcor[nuclei][isotope].gety()[fit_index_min:fit_index_max], sigma=graph_inntrack_effcor[nuclei][isotope].get_yerrors()[fit_index_min:fit_index_max], p0=[0, 0, 0])

            fitinntrk[nuclei][isotope] = poly_func(np.log(kNucleiBinsRebin_center), *fit_coeffs)
            fitinntrk[nuclei][isotope][transit_binindex+2:] = fitinntrk[nuclei][isotope][transit_binindex+1] * np.ones_like(fitinntrk[nuclei][isotope][transit_binindex+2:])
            
            #spline_inntrk[nuclei][isotope] = get_spline_from_graph(graph_inntrack_effcor[nuclei][isotope])
            spline_inntrk[nuclei][isotope] = UnivariateSpline(np.log(graph_inntrack_effcor[nuclei][isotope].getx()), graph_inntrack_effcor[nuclei][isotope].gety(), w=1/(graph_inntrack_effcor[nuclei][isotope].yerrs), k=3, s=5)
            #spline_inntrk[nuclei][isotope] =  make_interp_spline(graph_inntrack_effcor[nuclei][isotope].getx(), graph_inntrack_effcor[nuclei][isotope].gety(), k=3)
            #splinefit_inntrk = spline_inntrk[nuclei][isotope](graph_inntrack_effcor[nuclei][isotope].getx())
            #ax2.plot(graph_inntrack_effcor[nuclei][isotope].getx(), splinefit_inntrk, color="black")
            #fitinntrk[nuclei][isotope] = inntrk_func(np.log(kNucleiBinsRebin_center), fit_coreffs, graph_inntrack_effcor[nuclei][isotope].yvalues[27], graph_inntrack_effcor[nuclei][isotope].xvalues[27])
            #plot_curvefit(figure, ax2, graph_inntrack_effcor[nuclei][isotope], poly_func, fitinntrk[nuclei][isotope], label="inntrk")
            ax2.plot(kNucleiBinsRebin_center, fitinntrk[nuclei][isotope])
        ax2.sharex(ax1)
        ax2.set_xlim([1.9, 1300])
        ax2.set_xticks([2, 5, 10, 30,  100,  300, 1000])
        ax1.text(0.05, 0.95, "Inner Tracking", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", weight='bold') 
        ax2.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
        ax2.set_ylabel("ISS/MC")
        ax1.sharex(ax2)
        ax1.legend(loc='lower right')
        ...
        ax1.get_yticklabels()[0].set_visible(False)
        savefig_tofile(figure, args.resultdir, f"hist_inntrk_{nuclei}_effcor", show=True)
    
    #inntrk efficiency comparison
    eff_cor_inntrk_jw = data_jiahui_eff[2][4]
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
    ax2.grid(linewidth=1)
    graph_inntrk = slice_graph(graph_inntrack_effcor["Be"]["Be7"], graph_inntrack_effcor["Be"]["Be7"].get_index(1.9), graph_inntrack_effcor["Be"]["Be7"].get_index(20))
    plot_graph_eff(figure, ax1, graph_inntrk, "Be7", legend="Be7",  ylabel="Efficiency correction")
    fit_inntrk_eff = fitinntrk["Be"]["Be7"]
    ax1.plot(kNucleiBinsRebin_center, eff_cor_inntrk_jw(np.log(kNucleiBinsRebin_center)), "-", color="black", label="J.W")
    ax1.plot(kNucleiBinsRebin_center, fit_inntrk_eff, "-", color="tab:orange", label="fit")
    #ax1.plot(kNucleiBinsRebin_center, spline_inntrk["Be"]["Be7"](np.log(kNucleiBinsRebin_center)), "-", color="tab:orange", label="fit")
    
    #ax2.plot(kNucleiBinsRebin_center, spline_inntrk["Be"]["Be7"](np.log(kNucleiBinsRebin_center))/eff_cor_inntrk_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")
    ax2.plot(kNucleiBinsRebin_center, fit_inntrk_eff/eff_cor_inntrk_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")
    ax1.set_ylim([0.95, 1.05])
    ax2.set_ylim([0.996, 1.005])
    ax1.set_xscale("log")
    ax2.set_xscale("log")
    ax2.set_xlim([1.9, 1300])
    ...
    ax2.text(0.05, 0.95, "Difference within 0.3%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="black", weight='bold') 
    
    ax1.legend()
    savefig_tofile(figure, args.resultdir, f"hist_compare_jw_inntrk_effcor", show=True)

    #BZ efficiency
    eff_cor_bz_jw = data_jiahui_eff[2][0]
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
    graph_bz = dict_eff_cor["Be"]["Be7"]["bz"]
    plot_graph_eff(figure, ax1, graph_bz, "Be7", legend="Be7", ylabel="Efficiency correction")
    ...
    ax1.plot(graph_bz.xvalues, fit_bz_eff, "-", color="tab:orange", label="fit")
    ...
    ax2.text(0.05, 0.95, "Difference within 0.4%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="black", weight='bold')
    set_plot_defaultstyle(ax1)
    set_plot_defaultstyle(ax2)
    plt.subplots_adjust(hspace=.0)
    ...
    #plot_bz_eff
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
    plot_graph_eff(figure, ax1, dictgraph_eff_iss["Be"]["bz"], "ISS", legend="ISS")
    for iso in isotopes["Be"]:
        plot_graph_eff(figure, ax1, dictgraph_eff_mc["Be"][iso]["bz"], iso, legend=f"{iso}")
        plot_graph_eff(figure, ax2, dict_eff_cor["Be"][iso]["bz"], iso)
        fit_bz_effcor = poly_func(np.log(kNucleiBinsRebin_center), *dict_polyfitpars_effcor["Be"][iso]["bz"])
        fit_bz_effcor[dict_eff_cor["Be"][iso]["bz"].get_index(108):] = fit_bz_effcor[dict_eff_cor["Be"][iso]["bz"].get_index(108)] * np.ones_like(fit_bz_effcor[dict_eff_cor["Be"][iso]["bz"].get_index(108):]) 
        ax2.plot(kNucleiBinsRebin_center, fit_bz_effcor, "-", color=color_iso[isotope])
        
    ax1.set_ylim(cutplotlim["bz"])
    ax2.set_ylim(effcorplotlim["bz"])
    ax1.text(0.1, 0.95, f"BZ", fontsize=FONTSIZE_BIG, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold") 
    ax1.legend(loc='lower right')
    ax2.set_xlim([1.9, 1300])
    ax2.set_xticks([2, 5, 10, 30,  100,  300, 1000])
    ax2.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ...
    #plt.grid()
    ax1.get_yticklabels()[0].set_visible(False)
    savefig_tofile(figure, args.resultdir, f"hist_Be_bz_rig_efficiency2", show=True)
    ...
    #tof q efficiency
    eff_cor_utof_q_jw = data_jiahui_eff[2][2]
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
    graph_utof_q = dict_eff_cor["Carbon"]["C12"]["tof"]
    plot_graph_eff(figure, ax1, graph_utof_q, "C12", legend="C12", ylabel="Efficiency correction")
    fit_utof_q_eff = poly_func(np.log(kNucleiBinsRebin_center), *dict_polyfitpars_effcor["Carbon"]["C12"]["tof"])
    #fit_bz_eff[graph_utof_q.get_index(108):] = fit_bz_eff[graph_bz.get_index(108)] * np.ones_like(fit_bz_eff[graph_utof_q.get_index(108):]) 
    ax1.plot(graph_utof_q.xvalues, eff_cor_utof_q_jw(np.log(kNucleiBinsRebin_center)), "-", color="black", label="J.W")
    ax1.plot(graph_utof_q.xvalues, fit_utof_q_eff, "-", color="tab:orange", label="fit")
    ax2.plot(graph_utof_q.xvalues, fit_utof_q_eff/eff_cor_utof_q_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")
    ...
    ax2.set_xlabel(RIG_XLABEL)
    ax2.set_ylabel("this/J.W")
    ax2.text(0.05, 0.95, "Difference within 0.05%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="black", weight='bold') 
    ax1.text(0.05, 0.95, "UpTof Q", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold") 
    plt.subplots_adjust(hspace=.0)
    savefig_tofile(figure, args.resultdir, f"hist_compare_jw_utofq_effcor", show=True)
    
    #open jiahui bz efficiency csv file:
    pd_jiahui_trigger = pd.read_csv("/home/manbing/Documents/Data/jiahui/efficiencies/trigger_eff_c.csv",  sep='\s+', header=0)
    print(pd_jiahui_trigger)
    graph_trigger_jiahui = MGraph(xvalues=pd_jiahui_trigger["bin_center[GV]"], yvalues=pd_jiahui_trigger["dt"]/pd_jiahui_trigger["C12"],
                             yerrs=get_ratio_err(pd_jiahui_trigger["dt"], pd_jiahui_trigger["C12"], pd_jiahui_trigger["dt_err"], pd_jiahui_trigger["C12_err"]))
    #trigger
    eff_cor_trigger_jw = data_jiahui_eff[2][1]
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
    graph_trigger = dict_eff_cor["Carbon"]["C12"]["trigger"]
    plot_graph_eff(figure, ax1, graph_trigger, "C12", legend="C12", ylabel="Efficiency correction")
    fit_trigger_eff = poly_func(np.log(kNucleiBinsRebin_center), *dict_polyfitpars_effcor["Carbon"]["C12"]["trigger"])
    #fit_bz_eff[graph_trigger.get_index(108):] = fit_bz_eff[graph_bz.get_index(108)] * np.ones_like(fit_bz_eff[graph_trigger.get_index(108):])
    ax1.plot(kNucleiBinsRebin_center, fit_trigger_eff, "-", color="tab:orange")
    plot_graph_eff(figure, ax1, graph_trigger_jiahui, "C12", legend="Jiahui", ylabel="Efficiency correction", col="black")

    ax1.plot(kNucleiBinsRebin_center, eff_cor_trigger_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")

    ax2.plot(kNucleiBinsRebin_center, fit_trigger_eff/eff_cor_trigger_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")
    ax1.set_ylim([0.95, 1.05])
    ax2.set_ylim([1.0, 1.001])
    ax1.set_xscale("log")
    ax2.set_xscale("log")
    ax2.set_xlim([1.9, 1300])
    ax2.set_xticks([2, 5,  10, 30,  80,  300, 1000])
    ...
    ax2.set_xlabel(RIG_XLABEL)
    ax2.set_ylabel("this/J.W")
    ax1.text(0.05, 0.95, "Trigger", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    ax2.text(0.05, 0.95, "Difference within 0.1%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="black", weight='bold')
    set_plot_defaultstyle(ax1)
    set_plot_defaultstyle(ax2)
    ax2.grid(linewidth=2)
    plt.subplots_adjust(hspace=.0)
    savefig_tofile(figure, args.resultdir, f"hist_compare_jw_trigger_effcor", show=True)
    ...
    polyfit_l1q_eff = get_nppolyfit(np.log(kNucleiBinsRebin_center), spline_l1q_eff(kNucleiBinsRebin_center), 2)
    eff_cor_l1q_jw = data_jiahui_eff[2][3]
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))    
    figure.subplots_adjust(left= 0.12, right=0.96, bottom=0.1, top=0.95) 
    ax1.plot(kNucleiBinsRebin_center, eff_cor_l1q_jw(np.log(kNucleiBinsRebin_center)), "-", color="black", label="J.W")
    ax1.plot(kNucleiBinsRebin_center, polyfit_l1q_eff(np.log(kNucleiBinsRebin_center)), "-", color="tab:orange", label="fit")
    ax2.plot(kNucleiBinsRebin_center, polyfit_l1q_eff(np.log(kNucleiBinsRebin_center))/eff_cor_l1q_jw(np.log(kNucleiBinsRebin_center)), "-", color="black")
    ax1.set_ylim([0.95, 1.03])
    ax2.set_ylim([0.99, 1.01])
    ax1.set_xscale("log")
    ax2.set_xscale("log")
    ax2.set_xlim([1.9, 1400])
    ax2.set_xticks([2, 5,  10, 30,  80,  300, 1000])
    ax2.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ax1.get_yticklabels()[0].set_visible(False)
    ax1.sharex(ax2)
    ...
    set_plot_defaultstyle(ax1)
    set_plot_defaultstyle(ax2)
    ax2.text(0.05, 0.95, "Difference within 0.5%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="black", weight='bold') 
    ...
    for iso in isotopes["Be"]:        
        ...
        ax1.plot(kNucleiBinsRebin_center, polyfit_l1q_eff(np.log(kNucleiBinsRebin_center)), '-', label="l1q upper", color="magenta") 
        ax1.plot(kNucleiBinsRebin_center, fitinntrk["Be"][iso], "-", color="tab:orange", label="inner track")
        ax1.plot(graph_bz.xvalues, fit_bz_eff, "-", color="blue", label="BZ")
        ...
        fit_l1q_eff = polyfit_l1q_eff(np.log(kNucleiBinsRebin_center))
        
        totalcor = fit_bz_eff* fit_trigger* fit_tof * fitinntrk["Be"][iso] *fit_l1q_eff
        print("totalcor:", totalcor)
        ...
        ax1.text(0.05, 0.95, "L1MaxQInner", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold") 
        ax1.legend()
        savefig_tofile(figure, args.resultdir, f"hist_{iso}_average_flux_effcor", show=True)

        spline_tot_eff = UnivariateSpline(np.log(kNucleiBinsRebin_center), totalcor, k=3, s=3)
        y_tot_eff = spline_tot_eff(kNucleiBinsRebin_center)
        print("y_tot_eff:", y_tot_eff)
        with open(os.path.join(args.resultdir, f'spline_{iso}_nucleiselection_effcor.pickle'), 'wb') as f:
            pickle.dump(spline_tot_eff, f)
