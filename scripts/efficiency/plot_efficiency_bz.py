...
def get_efficiency(nuclei, filename, datatype, variable):
    ...
    with uproot.open(filename) as rootfile:
        for i, cut in enumerate(studyeff):
            print(nuclei, filename, histnameden[cut])
...
def plot_graph_eff(figure, plot, graph_eff, isotope=None, legend=None, col=None, ylabel=None):
    plot1d_errorbar_v2(figure, plot,  graph_eff.getx(), graph_eff.gety(), err=graph_eff.get_yerrors(), label_x="Rigidity (GV)", label_y="Efficiency" if ylabel is None else ylabel, label=legend, color=color_iso[isotope] if col is None else col, style=".", legendfontsize=FONTSIZE_BIG, setlogx=True, setlogy=False, setscilabelx=False, setscilabely=False, markersize=20)
...
def main():
    ...
    parser.add_argument("--resultdir", default="plots/bz", help="Directory to store plots and result files in.")
    parser.add_argument("--variable", default="Rigidity", help="dataname for the output file and plots.")
    parser.add_argument("--savedatadir", default="plots/bz", help="dataname for the output file and plots.")
    ...
    pd_jiahui_bz = pd.read_csv("/home/manbing/Documents/Data/jiahui/efficiencies/l1_bz_eff.csv",  sep='\s+', header=0)
    graph_bz_jiahui = MGraph(xvalues=pd_jiahui_bz["bin_center[GV]"], yvalues=pd_jiahui_bz["dt"]/pd_jiahui_bz["Be_mix"],
                             yerrs=get_ratio_err(pd_jiahui_bz["dt"], pd_jiahui_bz["Be_mix"], pd_jiahui_bz["dt_err"], pd_jiahui_bz["Be_mix_err"]))

    graph_bz_jiahui_iss = MGraph(xvalues=pd_jiahui_bz["bin_center[GV]"], yvalues=pd_jiahui_bz["dt"], yerrs=pd_jiahui_bz["dt_err"])
    graph_bz_jiahui_mc = MGraph(xvalues=pd_jiahui_bz["bin_center[GV]"], yvalues=pd_jiahui_bz["Be_mix"], yerrs=pd_jiahui_bz["Be_mix_err"])
    ...
    np.savez(os.path.join(args.savedatadir, f"graph_bz_eff.npz"), **df_graph_effissmc) 
    #BZ efficiency
    
    init_ratio = {'Be7': 0.6, 'Be9': 0.3, 'Be10': 0.1, 'Li6': 0.5, 'Li7': 0.5, 'B10':0.5, 'B11':0.5}
    ...
    graph_bz = MGraph(xvalues= dict_eff_cor["Be"]["Be7"]["bz"].xvalues, yvalues=np.zeros_like(dict_eff_cor["Be"]["Be7"]["bz"].xvalues), yerrs=np.zeros_like(dict_eff_cor["Be"]["Be7"]["bz"].xvalues))
    for iso in ISOTOPES["Be"]:
        graph_bz = graph_bz + scale_graph(dict_eff_cor["Be"][iso]["bz"], init_ratio[iso])
    
    plot_graph_eff(figure, ax1, graph_bz, "Be7", legend="this", ylabel="ISS/MC(Be mix)")
    ployfit_pars  = getpars_curvefit_poly(graph_bz, 3)

    fit_bz_eff = poly_func(np.log(graph_bz.getx()[:]), *ployfit_pars)
    start_index = graph_bz.get_index(2.0)
    break_index = graph_bz.get_index(1400)
    fit_bz_eff[break_index:] = fit_bz_eff[break_index] * np.ones_like(fit_bz_eff[break_index:])

    spline_bz_effcor = UnivariateSpline(np.log(graph_bz.xvalues[start_index:break_index]), graph_bz.yvalues[start_index:break_index], k=3, s=100)
    bz_eff_spline = spline_bz_effcor(np.log(graph_bz.xvalues[start_index:break_index+1]))
    dict_graph_bz = dict()
    graph_bz.add_to_file(dict_graph_bz, f"graph_bz_effcor_{nuclei}")
    np.savez(os.path.join(args.savedatadir, f"graph_bz_effcor.npz"), **dict_graph_bz)
    save_spline_to_file(spline_bz_effcor, args.savedatadir, "spline_bz_effcor.pickle")
    
    print(graph_bz.xvalues)
    print(graph_bz.yvalues)
    print(bz_eff_spline)
    
    #ax1.plot(graph_bz.xvalues, fit_bz_eff, "-", color="tab:orange")
    ax1.plot(graph_bz.xvalues[start_index:break_index+1], bz_eff_spline, "-", color="tab:orange")
    plot_graph_eff(figure, ax1, graph_bz_jiahui, "Be7", col='grey', legend="Jiahui", ylabel="ISS/MC(Be mix)")
    ax2.plot(graph_bz.xvalues[start_index:break_index+1], bz_eff_spline/eff_cor_bz_jw(np.log(graph_bz.xvalues)[start_index:break_index+1]), "-", color="red", label="ratio - fit")
    ax2.plot(graph_bz.xvalues[start_index:break_index+1], graph_bz.yvalues[start_index:break_index+1]/graph_bz_jiahui.yvalues[start_index:break_index+1], ".", markersize=20, color="black", label="ratio - data")
    ax1.plot(graph_bz.xvalues[start_index:break_index+1], eff_cor_bz_jw(np.log(graph_bz.xvalues[start_index:break_index+1])), "-", color="grey")
    ...
    #plot_bz_eff
    figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
    plot_graph_eff(figure, ax1, dictgraph_eff_iss["Be"]["bz"], "ISS", legend="ISS Be")
    for iso in isotopes["Be"]:
        plot_graph_eff(figure, ax1, dictgraph_eff_mc["Be"][iso]["bz"], iso, legend=f"MC {iso}")
        ...
        ax2.plot(kNucleiBinsRebin_center, fit_bz_effcor, "-", color=color_iso[iso])

    plot1d_errorbar_v2(figure, ax1,  graph_bz_jiahui_iss.getx(), graph_bz_jiahui_iss.gety(), err=graph_bz_jiahui_iss.get_yerrors(), label="Jiahui ISS", color="grey", style=".", markersize=20)
    plot1d_errorbar_v2(figure, ax1,  graph_bz_jiahui_mc.getx(), graph_bz_jiahui_mc.gety(), err=graph_bz_jiahui_mc.get_yerrors(), label="Jiahui MCMix", color="magenta", style=".", markersize=20)
    plot_graph_eff(figure, ax2, graph_bz_jiahui, "Be7", col='grey', legend="Jiahui", ylabel="ISS/MC")
