...
color_iso = {"ISS": "black", "Be7": "tab:blue", "Be9": "tab:orange", "Be10": 'tab:green', "C12": 'tab:orange'}
color_cut = {"l1q": "magenta", "tof": "tab:red", "inntof": "tab:orange", "inncutoff": "tab:orange", "pk": "brown", "bz": "tab:blue", "trigger": "green", "total": "black"}
...
def get_efficiency(nuclei, filename, datatype, variable):
    ...
    histnameden = {"l1q" : f"h_l1q_den_{znum}_0", "tof": f"h_tof_den_{znum}_0", "inntof":f"h_inn_den_{znum}_0_0", "inncutoff": f"h_inn_den_{znum}_0_1", "pk": f"h_pk_den_{znum}_0", "bz":f"h_bz_den_{znum}_0", "trigger": f"h_trigger_{znum}_0_ub2"}
    histnamenum = {"l1q" : f"h_l1q_num_{znum}_0", "tof": f"h_tof_num_{znum}_0", "inntof": f"h_inn_num_{znum}_0_0", "inncutoff":f"h_inn_num_{znum}_0_1",  "pk": f"h_pk_num_{znum}_0", "bz":f"h_bz_num_{znum}_0", "trigger": f"h_trigger_{znum}_0_phy2"}
    ...
    with uproot.open(filename) as rootfile:
        for i, cut in enumerate(studyeff):
            ...
            npass, binedges = hist_pass.to_numpy(flow=False)
            ntot, binedges = hist_tot.to_numpy(flow=False)
            print(npass)
            print(ntot)
...
def plot_graph_eff(figure, plot, graph_eff, isotope, legend=None, col=None):
...
def plot_curvefit(figure, plot, graph, func, pars, col=None):
    plot.plot(graph.getx()[:], func(graph.getx()[:], *pars), '-', label='Fit', color=col)
    ...
    for nuclei in nucleus:
        ...
        for i, isotope in enumerate(isotopes[nuclei]):
            ...
            for cut in studyeff:
                ...
                initial_guess = np.zeros(4) # Initial guess for the polynomial coefficients
                fitrange = [1, -1]
                xlim = fitrange[0]
                ylim = fitrange[1]
