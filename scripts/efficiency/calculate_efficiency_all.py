...
cutplotlim = {"l1q": [0.95, 1.02], "tof": [0.9, 1.02], "inntof": [0.7, 0.95], "inncutoff": [0.7, 0.95], "pk": [0.7, 1.0], "bz" : [0.6, 0.9], "trigger": [0.99, 1.005]}
effcorplotlim = {"l1q": [0.95, 1.05], "tof": [0.9, 1.1], "inntof": [0.9, 1.1], "inncutoff": [0.9, 1.1], "pk": [0.9, 1.1], "bz" : [0.9, 1.1], "trigger": [0.95, 1.02]}
...
def plot_graph_eff(figure, plot, graph_eff, isotope=None, legend=None, col=None):
    plot1d_errorbar_v2(figure, plot,  graph_eff.getx(), graph_eff.gety(), err=graph_eff.get_yerrors(), label_x="Rigidity", label_y="Efficiency", legend=legend if legend is not None else f'{isotope}', col=color_iso[isotope] if col is None else col, style=".", legendfontsize=FONTSIZE_BIG, setlogx=True, setlogy=False, setscilabelx=False, setscilabely=False)
...
def getpars_curvefit_poly(datagraph, deg):
    ...
    fit_coeffs, _ = curve_fit(poly_func, datagraph.getx()[:], datagraph.gety()[:], sigma=datagraph.get_yerrors()[:], p0=initial_guess)
    return fit_coeffs
...
def main():
    ...
    for nuclei in nucleus:
        ...
        for cut in studyeff:  
            ...
            plot_graph_eff(figure, ax1, dictgraph_eff_iss[nuclei][cut], "ISS")
            for i, isotope in enumerate(isotopes[nuclei]):
                plot_graph_eff(figure, ax1, dictgraph_eff_mc[nuclei][isotope][cut], isotope)
                ...
                #ax2.plot(dict_eff_cor[nuclei][isotope][cut].getx()[:], poly_func(dict_eff_cor[nuclei][isotope][cut].getx()[:], *dict_polyfitpars_effcor[nuclei][isotope][cut]), 'r-', label='Fit')
        ...
        for i, isotope in enumerate(isotopes[nuclei]):
            ...
            plot_graph_eff(figure, ax2, graph_inntrack_effcor[nuclei][isotope], isotope)
            fit_coeffs, _ = curve_fit(poly_func, graph_inntrack_effcor[nuclei][isotope].getx()[:], graph_inntrack_effcor[nuclei][isotope].gety()[:], sigma=graph_inntrack_effcor[nuclei][isotope].get_yerrors()[:], p0=initial_guess)

            fitinntrk[nuclei][isotope] = inntrk_func(kNucleiBinsRebin_center, fit_coeffs, 0.96)
    ...
    for iso in isotopes["Be"]:
        ...
        print(dict_eff_cor["Be"][iso]["bz"].getx())
    ...
    for iso in isotopes["Be"]:        
        ...
        #plot_curvefit(figure, ax1, dict_eff_cor["Be"][iso]["inn"], poly_func, dict_polyfitpars_effcor["Be"][iso]["inn"], col=color_cut['inn'])
        ...
        #fit_bz = get_nppolyfit(dict_eff_cor["Be"][iso]["bz"], 2)
        ...
        #fit_trigger = get_nppolyfit(dict_eff_cor["Carbon"]["C12"]["trigger"], 2)
        ...
        #totalcor = fit_bz(kNucleiBinsRebin_center[2:-3]) * fit_trigger(kNucleiBinsRebin_center[2:-3]) * fit_tof(kNucleiBinsRebin_center[2:-3])  * fitinntrk["Be"][iso][2:-3]
        print(totalcor)

        if iso == "Be7":
            func_totcor = np.polyfit(kNucleiBinsRebin_center, totalcor, 3)
            np.savetxt("trees/eff_corr/fit_params_totcor.txt", func_totcor)
            print(func_totcor)
        
        #y_totalcor = totalcor(dict_eff_cor["Be"][iso]["bz"].getx())
        ax1.set_ylabel("ISS/MC")
        ax1.plot(dict_eff_cor["Be"][iso]["bz"].getx(), totalcor, '-',  color="black")
        ax1.set_xscale("log")
        ax1.set_xlim([2, 100])
