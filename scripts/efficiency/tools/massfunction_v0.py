...
class TofInverseMassFunctionFit(InverseMassFunctionFit):
    ...
    def make_mass_function_simultaneous(self,  drawiso="All"):
        ...
        def mass_function(x, *pars):
            ...
            asy_factor_a, asy_factor_b, asy_factor_c = pars[8:11]  
            ...
            ########################################################
            #temporate solution for only Be
            #########################################################
            pars_mean_scale = {'Be9': np.array([0.77976029, 0.00218487, 0.00102051]),
                               'Be10': np.array([7.01888721e-01, 9.03767144e-04, -4.74816286e-04])}
            
            #pars_asyfactor_scale_be10 = np.array([1.03201789, 0.06197469, 0.01968635])
            ...
            for i, n in enumerate(norm):
                ...
                if i == 0:
                    ...
                    rigsigma_factor = 1.0
                    #asyfactor_scale = poly(np.log(energy), *pars_asyfactor_scale)
                    asyfactor_scale = 1.0
        ...
        if self.is_constraint: 
            parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc',  'fraccore', 'sigma_ratio', 'asy_factor_a', 'asy_factor_b', 'asy_factor_c' ] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in isotopes_atom_num[:-1] for ibin in range(num_energybin)]
        else:
            parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc',  'fraccore', 'sigma_ratio', 'asy_factor_a', 'asy_factor_b', 'asy_factor_c' ] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in isotopes_atom_num for ibin in range(num_energybin)]

        mass_function.func_code = make_func_code(parnames)
        return mass_function

    def get_polypars_index(self):
        parindex = {'mua': 0, 'mub':1, 'muc':2,  'siga':3, 'sigb':4, 'sigc':5,  'fraccore':6, 'sigma_ratio':7,  'asy_factor_a':8, 'asy_factor_b':9,'asy_factor_c':10}
        return parindex
