...
def plot_fitmc_pars(parname, detectors, dict_fit_results_be7, massfit, poly_initp0, plotdir, ylabel, yrange, nuclei, isotope, figname=None, legend=None):
    ...
    if figname is not None:
        savefig_tofile(fig, plotdir, f"{nuclei}fit_{parname}_mc_{polydeg}deg_{legend}_{figname}", show=True)
    else:
...
def plot_fitmc_compare_isopars(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    ...
    for dec in detectors:
        ...
        ratio_li6li7[dec] = graph_init[dec]["Li7"]/graph_init[dec]["Li6"]
        ratio_li6li7[dec].add_to_file(df_sigma_ratio, f'graph_mass_sigma_7to6_{dec}')
        
        plot_graph(fig, ax2, ratio_li6li7[dec], color=ISOTOPES_COLOR['Li7'], style="EP", xlog=True, ylog=False, scale=None, markersize=22, label='Li7/Li6')
        if dec =="Tof":
    ...
    np.savez(os.path.join(plotdir, f"df_sigma_ratio_{parname}.npz"), **df_sigma_ratio)
    ax1.legend(loc='center right', fontsize=FONTSIZE-2) 
    if parname == "mean" or parname == "sigma":
        ax2.axhline(y=6.0/7.0, color='orange', linestyle='--')
        #ax2.axhline(y=ISOTOPES_MASS['Li6']/ISOTOPES_MASS['Li7'], color='green', linestyle='--')
        label_ref = mlines.Line2D([], [], linestyle='--', color=ISOTOPES_COLOR['Li7'], label='ref m6/m7')
        legend2 = ax2.legend(handles=[label_ref], loc='upper left', bbox_to_anchor=(0.05, 1.02), fontsize=20)
        ax2.add_artist(legend2)

    if parname == "mean":
        label_com = mlines.Line2D([], [], linestyle='--', marker = "o",  markersize= 14, color=ISOTOPES_COLOR['Li6'], label=r"$\mathrm{\mu_{7}}$/$\mathrm{\mu_{6}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)

    if parname == "sigma":
        label_com = mlines.Line2D([], [],  marker = "o",  markersize= 12, color=ISOTOPES_COLOR['Li7'], label=r"$\mathrm{\sigma_{7}}$/$\mathrm{\sigma_{6}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)
...
def plot_fitmc_compare_isopars_rigreso(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    ...
    for dec in detectors:
        ...
        ratio_li7li6[dec] = graph_init[dec]["Li7"]/graph_init[dec]["Li6"]
        ratio_li7li6[dec].add_to_file(df_sigma_ratio, f'graph_mass_sigma_9to7_{dec}')
    
        plot_graph(fig, ax2, ratio_li7li6[dec], color=ISOTOPES_COLOR['Li7'], style="EP", xlog=True, ylog=False, scale=None, markersize=22, label='Li7/Li6')
    ...
    ax1.legend(loc='center right', fontsize=FONTSIZE-2)  
    if parname == "mean":
        label_com = mlines.Line2D([], [], linestyle='--', marker = "o",  markersize= 12, color=ISOTOPES_COLOR['Li7'], label=r"$\mathrm{\mu_{6}}$/$\mathrm{\mu_{7}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)
        
    if parname == "sigma":
        label_com = mlines.Line2D([], [],  marker = "o",  markersize= 11, color=ISOTOPES_COLOR['Li7'], label=r"$\mathrm{\sigma_{7}}$/$\mathrm{\sigma_{6}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)
    ...
    ax1.legend(loc="lower right", fontsize=28)
    ax1.text(0.07, 0.98, f"Li MC", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold") 
    ax1.set_ylim(yrange[parname])
    ax1.fill_betweenx(np.linspace(yrange[parname][0], yrange[parname][1], 100), ANALYSIS_RANGE_RIG['Li']['NaF'][0], ANALYSIS_RANGE_RIG['Li']['NaF'][1], alpha=0.1, color=DETECTOR_COLOR['NaF'])  
    plt.subplots_adjust(hspace=.0)
    #ax1.get_yticklabels()[0].set_visible(False)
    ax1.set_xticklabels([])
    set_plot_defaultstyle(ax1)
...
def plot_fitdata_pars(parname, detectors, massfit, fit_parameters, init_parameters, polypars_initp0, ylabel, yrange, nuclei, plotdir, plot_mc=True, figname=None):
    parname_s = {'Tof': {'mean': 'mua', 'sigma':'siga' , 'fraccore': 'fraccore', 'asy_factor': 'asy_factor_a', 'sigma_ratio': 'sigma_ratio'},
                 'NaF': {'mean': 'mua', 'sigma':'siga' , 'fraccore': 'fraccore', 'asy_factor': 'asy_factor', 'sigma_ratio': 'sigma_ratio'},
                 'Agl': {'mean': 'mua', 'sigma':'siga' , 'fraccore': 'fraccore', 'asy_factor': 'asy_factor_a', 'sigma_ratio': 'sigma_ratio'}}
    ...
    for dec in detectors:
        ...
        if plot_mc:
            ax1.plot(xval, yinit, '--', color='black', label=f'{dec} guess')
...
def plot_fitmc_compare_isopars_betareso(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    ...
    for dec in detectors:
        ...
        ratio_li7li6[dec] = graph_init[dec]["Li7"]/graph_init[dec]["Li6"]
        ratio_li7li6[dec].add_to_file(df_sigma_ratio, f'graph_mass_sigma_7to6_{dec}')
    
        plot_graph(fig, ax2, ratio_li7li6[dec], color=ISOTOPES_COLOR['Li7'], style="EP", xlog=True, ylog=False, scale=None, markersize=22, label='Li7/Li6')

        ax1.legend(loc='center right', fontsize=FONTSIZE-2)  
        if parname == "mean":
            label_com = mlines.Line2D([], [], linestyle='--', marker = "o",  markersize= 12, color=ISOTOPES_COLOR['Li7'], label=r"$\mathrm{\mu_{7}}$/$\mathrm{\mu_{6}}$")
            legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
            ax2.add_artist(legend3)
