...
class AglInverseMassFunctionFit(InverseMassFunctionFit):
    ...
    def make_mass_function_simultaneous(self, drawiso="All"):
        ...
        def mass_function(x, *pars):
            ...
            scale_sigma_ratio = np.array([1.0, 1.03, 1.055])


            scale_factors_mean = {"Be9": np.array([0.78405044, -0.00586313, 0.00175498]),
                                  "Be10": np.array([0.70541796, -0.00365599,  0.00108003])}
            ...
            for i, n in enumerate(norm):
                ...
                if i == 0:
                    ...
                    mean_scale = poly(np.log(energy), *scale_factors_mean_enloss[self.isotopes[i]])
                    sigma_scale = isofactor / rigsigma_factor
                    
                coregaus = gaussian(mass, mean * mean_scale, sigma * sigma_scale)
                asygaus = asy_gaussian(mass, mean * mean_scale,  sigma_ratio * scale_sigma_ratio[i] * sigma * sigma_scale, asy_factor * scale_asyfactor[i])
        ...
        if self.is_constraint:
            parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc', 'sigd', 'fraccore_a', 'fraccore_b', 'fraccore_c',  'sigma_ratio_a', 'sigma_ratio_b', 'sigma_ratio_c', 'asy_factor_a', 'asy_factor_b', 'asy_factor_c'] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in isotopes_atom_num[:-1]  for ibin in range(num_energybin)]
        else:
            parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc', 'sigd', 'fraccore_a', 'fraccore_b', 'fraccore_c', 'sigma_ratio_a', 'sigma_ratio_b',  'sigma_ratio_c', 'asy_factor_a', 'asy_factor_b', 'asy_factor_c'] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in isotopes_atom_num  for ibin in range(num_energybin)]
       
        mass_function.func_code = make_func_code(parnames)
        return mass_function
    
    def get_polypars_index(self):
        parindex = {'mua': 0, 'mub':1, 'muc':2, 'siga':3, 'sigb':4, 'sigc':5, 'sigd': 6, 'fraccore_a':7, 'fraccore_b':8, 'fraccore_c':9, 'sigma_ratio_a':10,  'sigma_ratio_b':11, 'sigma_ratio_c':12, 'asy_factor_a':13, 'asy_factor_b':14, 'asy_factor_c':15}
        return parindex
