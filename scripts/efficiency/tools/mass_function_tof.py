...
class InverseMassFunctionFit:
    ...
    def make_mass_function_simultaneous(self, is_constraint=True, drawiso="All"):
        ...
        parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc', 'fraccore', 'sigma_ratio_a', 'sigma_ratio_b', 'sigma_ratio_c', 'asy_factor_a', 'asy_factor_b', 'asy_factor_c' ] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in (isotopes_atom_num[:-1] if is_constraint else isotopes_atom_num)  for ibin in range(num_energybin)]
