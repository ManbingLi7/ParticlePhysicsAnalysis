...
class InverseMassFunctionFit(metaclass=abc.ABCMeta):
    ...
    def perform_fit(self, guess, fit_simultaneous=True, verbose=False, fixed_pars=None, lim_pars=None, parlim=None):
        ...
        if self.nonlinearconstraint and (parlim is not None):
            ...
            if self.detector  == 'Agl':
                ...
                print('upper-init: ', parlim['sigma'][1, :] - poly(np.log(x_energy), guess['siga'], guess['sigb'], guess['sigc']))
                print('init-lower: ', poly(np.log(x_energy), guess['siga'], guess['sigb'], guess['sigc']) - parlim['sigma'][0, :])

            #all_constraints = [constraint_mu, constraint_sig]
            all_constraints = [constraint_sig]
    ...
    def draw_fit_results_mc(self, fit_parameters, par_dicts, plotdir, fit_simultaneous=False, guess=None, x_label=None, figname=None, drawlog=False):
        ...
        for i, ibin in enumerate(range(self.fit_energy_binrange[0], self.fit_energy_binrange[1] + 1)):
            ...
            if fit_simultaneous:
                ...
                iso = self.isotopes[0]
                savefig_tofile(figure, plotdir, f"fitbin{ibin}_{self.detector}_binbybin_{figname}" if figname is not None else "fitbin{ibin}_{self.detector}_binbybin", show=False)
    ...
    def draw_fit_results_mc_betareso(self, fit_parameters, par_dicts, plotdir, fit_simultaneous=False, guess=None, x_label=None, figname=None):
        ...
        for i, ibin in enumerate(range(self.fit_energy_binrange[0], self.fit_energy_binrange[1] + 1)):
            ...
            fit_info_formatted = [f"\\fontsize{FONTSIZE}\\selectfont {info}" for info in fit_info]
            ax1.legend(handles, labels, title="\n".join(fit_info), frameon=False, fontsize=12)
            ax1.set_xticklabels([])
            ax1.get_yticklabels()[0].set_visible(False)
            ax2.set_ylim([-3.8, 3.8])
            ax1.set_ylim([0.1, 1.3 *np.max(counts)])
            ...
            if fit_simultaneous:
                ...
                iso = self.isotopes[0]
                savefig_tofile(figure, plotdir, f"fitbin{ibin}_{self.detector}_binbybin_{iso}_{figname}" if figname is not None else "fitbin{ibin}_{self.detector}_binbybin_{iso}", show=False)
