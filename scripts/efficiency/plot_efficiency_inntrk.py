...
from tools.plottools import plot1dhist, plot2dhist, plot1d_errorbar, FIGSIZE_MID, FIGSIZE_BIG, setplot_defaultstyle, format_order_of_magnitude, FONTSIZE, savefig_tofile, FONTSIZE_BIG, plot1d_errorbar_v2, tick_length, tick_labelsize, tick_width, set_plot_defaultstyle, set_plot_defaultstyle
...
def plot_comparison_twohist(figure=None, ax1=None, ax2=None, histA=None, histB=None, ratio=None, ratioerr=None, xlog=True, ylog=False, isotope=None, xlabel=None, ylabel=None, figname=None, save=False, cutname=None):
    ...
    ax2.sharex(ax1)
    if (save):
        savefig_tofile(figure, "plots/inntrk", f"hist_{figname}_rig_passtotalevent", show=False)
     
isotopes = {"Li": ["Li6", "Li7"], "Be": ["Be7", "Be9", "Be10"], "Boron": ["B10", "B11"], "Carbon": ["C12"]}
color_iso = {"ISS": "black", "Be7": "tab:orange", "Be9": "tab:blue", "Be10": 'tab:green', "C12": 'tab:orange', 'Li6': 'tab:blue', 'Li7': 'tab:orange', 'B10':'tab:blue', 'B11':'tab:orange'}
...
cutplotlim = {"l1q": [0.95, 1.02], "tof": [0.975, 1.005], "inntof": [0.85, 0.95], "inncutoff": [0.85, 0.95], "pk": [0.7, 1.0], "bz" : [0.65, 0.85], "trigger": [0.985, 1.005]}
...
def get_efficiency(nuclei, filename, datatype, variable):
    ...
    with uproot.open(filename) as rootfile:
        for i, cut in enumerate(studyeff):
            ...
            savefig_tofile(figure, "plots/inntrk", f"hist_{nuclei}{datatype}_{cut}_passevent", show=False)
            
    return dict_hist_eff

def plot_graph_eff(figure, plot, graph_eff, isotope=None, legend=None, col=None, ylabel=None, style=None):
    plot1d_errorbar_v2(figure, plot,  graph_eff.getx(), graph_eff.gety(), err=graph_eff.get_yerrors(), label_x="Rigidity (GV)", label_y="Efficiency" if ylabel is None else ylabel, label=legend, color=color_iso[isotope] if col is None else col, style="." if style is None else style, legendfontsize=FONTSIZE_BIG, setlogx=True, setlogy=False, setscilabelx=False, setscilabely=False, markersize=30)

def plot_graph_eff_mc(figure, plot, graph_eff, isotope=None, legend=None, col=None, ylabel=None, style=None):
    plot1d_errorbar_v2(figure, plot,  graph_eff.getx(), graph_eff.gety(), err=graph_eff.get_yerrors(), label_x="Rigidity (GV)", label_y="Efficiency" if ylabel is None else ylabel, label=legend, color=color_iso[isotope] if col is None else col, style='s', legendfontsize=FONTSIZE_BIG, setlogx=True, setlogy=False, setscilabelx=False, setscilabely=False, markersize=13)
...
def main():
    ...
    parser.add_argument("--filename_iss", default="/home/manbing/Documents/Data/data_BeP8/efficiency/anaflux_ISSP8_GBL.root", help="Path to root file to read tree from")
    #parser.add_argument("--filename_iss", default="/home/manbing/Documents/Data/data_BeP8/efficiency/anaISSP8_inntrk_eff.root", help="Path to root file to read tree from")
    ...
    parser.add_argument("--resultdir", default="plots/inntrk", help="Directory to store plots and result files in.")
    ...
    parser.add_argument("--savedatadir", default="plots/inntrk", help="dataname for the output file and plots.")
    ...
    filename_mc = {nuclei: {isotope: f'/home/manbing/Documents/Data/data_BeP8/efficiency/{isotope}MC_B1236_anaflux_GBL.root' for isotope in isotopes[nuclei]} for nuclei in nucleus}
    #filename_mc = {nuclei: {isotope: f'/home/manbing/Documents/Data/data_BeP8/efficiency/{isotope}MCP8_inntrk_eff.root' for isotope in isotopes[nuclei]} for nuclei in nucleus}
    
    graph_inntrack_eff_iss = dict()
    show_graph_inntrack_eff_iss = dict()
    graph_inntrack_eff_mc = {nuclei: dict() for nuclei in nucleus}
    graph_inntrack_eff_mc_mix = {}
    ...
    pd_jiahui_inntrk = pd.read_csv("/home/manbing/Documents/Data/jiahui/efficiencies/inner_eff.csv",  sep='\s+', header=0)
    print(pd_jiahui_inntrk)
    graph_inntrk_jiahui = MGraph(xvalues=pd_jiahui_inntrk["bin_center[GV]"], yvalues=pd_jiahui_inntrk["dt"]/pd_jiahui_inntrk["Be_mix"],
                             yerrs=get_ratio_err(pd_jiahui_inntrk["dt"], pd_jiahui_inntrk["Be_mix"], pd_jiahui_inntrk["dt_err"], pd_jiahui_inntrk["Be_mix_err"]))

    graph_inntrk_jiahui_iss = MGraph(xvalues=pd_jiahui_inntrk["bin_center[GV]"], yvalues=pd_jiahui_inntrk["dt"], yerrs=pd_jiahui_inntrk["dt_err"])
    graph_inntrk_jiahui_mc = MGraph(xvalues=pd_jiahui_inntrk["bin_center[GV]"], yvalues=pd_jiahui_inntrk["Be_mix"], yerrs=pd_jiahui_inntrk["Be_mix_err"])

    df_graph_effissmc = {}     
    for nuclei in nucleus:
        dictgraph_eff_iss[nuclei] =  get_efficiency('Be', args.filename_iss, "ISS", variable)
        ...
        for cut in studyeff:  
            ...
            for i, isotope in enumerate(isotopes[nuclei]):
                plot_graph_eff_mc(figure, ax1, dictgraph_eff_mc[nuclei][isotope][cut], isotope, legend=f"{isotope}")
        ...
        slice_xpoint_inncutoff = 14
        #plot the inner tracking efficiency:
        figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(16, 13))
        transit_binindex = dictgraph_eff_iss[nuclei]["inntof"].get_index(19)
        fit_index_min = dictgraph_eff_iss[nuclei]["inntof"].get_index(2.0)
        fit_index_max = dictgraph_eff_iss[nuclei]["inntof"].get_index(18)
        ...
        show_graph_inntrack_eff_iss[nuclei] = concatenate_graphs(slice_graph(dictgraph_eff_iss[nuclei]["inntof"], 1, 15), slice_graph(dictgraph_eff_iss[nuclei]["inncutoff"], 15, fit_index_max))

        #########################################################################################################
        if nuclei == 'Li':
            show_graph_inntrack_eff_iss[nuclei] = scale_graph(show_graph_inntrack_eff_iss[nuclei], 0.99)
        #########################################################################################################
        
        plot_graph_eff(figure, ax1, show_graph_inntrack_eff_iss[nuclei], "ISS", legend="ISS")
        
        #graph_inntrack_eff_mc_mix[nuclei] = MGraph(dictgraph_eff_iss[nuclei]["inntof"].xvalues, yvalues=np.zeros_like(dictgraph_eff_iss[nuclei]["inntof"].yvalues), yerrs=np.zeros_like(dictgraph_eff_iss[nuclei]["inntof"].yvalues))
        for i, isotope in enumerate(isotopes[nuclei]):
            graph_inntrack_eff_mc[nuclei][isotope] = concatenate_graphs(slice_graph(dictgraph_eff_mc[nuclei][isotope]["inntof"], 1, 14), slice_graph(dictgraph_eff_mc[nuclei][isotope]["inncutoff"], 15))                      
            #graph_inntrack_eff_mc_mix[nuclei] = graph_inntrack_eff_mc_mix[nuclei] + scale_graph(graph_inntrack_eff_mc[nuclei][isotope], init_ratio[isotope])

            graph_inntrack_effcor[nuclei][isotope] = concatenate_graphs(slice_graph(dict_eff_cor[nuclei][isotope]["inntof"] , 1, 14), slice_graph(dict_eff_cor[nuclei][isotope]["inncutoff"], 15))
            ...
            sliced_graph_inntrack_effcor = slice_graph(graph_inntrack_effcor[nuclei][isotope], fit_index_min, fit_index_max-1)
            
            plot_graph_eff_mc(figure, ax1, graph_inntrack_eff_mc[nuclei][isotope], isotope, legend=f"MC {isotope}")
            ...
            ####################to be removed######################################################################
            if nuclei == 'Li':
                sliced_graph_inntrack_effcor = scale_graph(sliced_graph_inntrack_effcor, 0.99)
                graph_inntrack_effcor[nuclei][isotope] = scale_graph(graph_inntrack_effcor[nuclei][isotope], 0.99)
            #########################################################################################################
            plot_graph_eff(figure, ax2, sliced_graph_inntrack_effcor, isotope)
           
            fit_coeffs, _ = curve_fit(poly_func, np.log(graph_inntrack_effcor[nuclei][isotope].getx()[fit_index_min:fit_index_max]), graph_inntrack_effcor[nuclei][isotope].gety()[fit_index_min:fit_index_max], sigma=graph_inntrack_effcor[nuclei][isotope].get_yerrors()[fit_index_min:fit_index_max], p0=[0, 0, 0, 0])
            fitinntrk[nuclei][isotope] = poly_func(np.log(kNucleiBinsRebin_center), *fit_coeffs)
            fitinntrk[nuclei][isotope][transit_binindex:] = fitinntrk[nuclei][isotope][transit_binindex-1] * np.ones_like(fitinntrk[nuclei][isotope][transit_binindex:])
            ...
            save_spline_to_file(spline_inntrk[nuclei][isotope], args.savedatadir, f"{nuclei}_spline_inntrk_effcor.pickle")
            ax2.plot(kNucleiBinsRebin_center, fitinntrk[nuclei][isotope])
        #plot1d_errorbar_v2(figure, ax1,  graph_inntrk_jiahui_iss.getx(), graph_inntrk_jiahui_iss.gety(), err=graph_inntrk_jiahui_iss.get_yerrors(), label="Jiahui", color="grey", style=".", markersize=25)
        #plot1d_errorbar_v2(figure, ax1,  graph_inntrk_jiahui_mc.getx(), graph_inntrk_jiahui_mc.gety(), err=graph_inntrk_jiahui_mc.get_yerrors(), label="Jiahui MCMix", color="magenta", style=".", markersize=23)
        #plot_graph_eff(figure, ax2, graph_inntrk_jiahui, "Be7",  ylabel="ISS/MC", col="grey")
        ax2.sharex(ax1)
        ax1.set_ylim([0.79, 0.9])
        ax2.set_xlim([1.9, 120])
        ax2.set_xticks([2, 5, 10, 30,  100,  300, 1000])
        ax1.text(0.05, 0.95, f"{nuclei} Inner Tracking", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", weight='bold') 
        ax2.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
