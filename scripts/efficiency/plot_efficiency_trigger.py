...
isotopes = {"Li": ["Li6", "Li7"], "Be": ["Be7", "Be9", "Be10"], "Boron": ["Bo10", "Bo11"], "Carbon": ["C12"], 'He': ['He4'], 'Oxygen': ['O16']}
...
chargename = {"He": 'z2', "Li": "z3", "Be": "z4", "Boron": "z5", "Carbon": "z6", 'Oxygen':'z8'}
...
def get_efficiency(nuclei, filename, datatype, variable):
    ...
    histnameden = {"l1q":       f"h_l1q_den_{znum}_0",
                   ...
                   "trigger":   f"h_trigger_{znum}_0_ub"}
    ...
    histnamenum = {"l1q" :      f"h_l1q_num_{znum}_0",
                   ...
                   "trigger":   f"h_trigger_{znum}_0_phy"}
    ...
    with uproot.open(filename) as rootfile:
        for i, cut in enumerate(studyeff):
            ...
            savefig_tofile(figure, "plots", f"hist_{nuclei}{datatype}_{cut}_passevent", show=True)
...
def getpars_curvefit_poly(datagraph, deg):
    ...
    fit_coeffs, _ = curve_fit(poly_func, np.log(datagraph.getx()[:]), datagraph.gety()[:], p0=initial_guess)
    return fit_coeffs
...
def main():
    ...
    parser.add_argument("--resultdir", default="plots/trigger", help="Directory to store plots and result files in.")
    ...
    parser.add_argument("--savedatadir", default="plots/trigger", help="dataname for the output file and plots.")    
    
    args = parser.parse_args()
    os.makedirs(args.resultdir, exist_ok=True)
    nucleus = ["He", "Carbon", "Oxygen"]
    ...
    graph_trigger_jiahui_iss = MGraph(xvalues=pd_jiahui_trigger["bin_center[GV]"], yvalues=pd_jiahui_trigger["dt"],
                                  yerrs=pd_jiahui_trigger["dt_err"])

    graph_trigger_jiahui_mc = MGraph(xvalues=pd_jiahui_trigger["bin_center[GV]"], yvalues=pd_jiahui_trigger["C12"],
                                  yerrs=pd_jiahui_trigger["C12_err"])
    ...
    for nuclei in nucleus:
        ...
        for i, isotope in enumerate(isotopes[nuclei]):        
            ...
            for cut in studyeff:
                ...
                ylim = fitrange[1]
                if "inn" not in cut:
                    dict_polyfitpars_effcor[nuclei][isotope][cut]  = getpars_curvefit_poly(dict_eff_cor[nuclei][isotope][cut], 1)
        ...
        for cut in studyeff:  
            ...
            for i, isotope in enumerate(isotopes[nuclei]):
                ...
                if "inn" not in cut:
                    plot_curvefit(figure, ax2, dict_eff_cor[nuclei][isotope][cut], poly_func, dict_polyfitpars_effcor[nuclei][isotope][cut], col="tab:orange")

            plot_graph_eff(figure, ax1, graph_trigger_jiahui_iss, "ISS", legend="Jiahui ISS", col="grey")
            plot_graph_eff(figure, ax1, graph_trigger_jiahui_mc, "C12", legend="Jiahui MC C12", col="magenta")
            plot_graph_eff(figure, ax2, graph_trigger_jiahui, "ISS", legend="Jiahui", col="grey")
    ...
    plot_graph_eff(figure, ax1, graph_trigger, "C12", legend="this", ylabel="Efficiency correction")
    ...
    plot_graph_eff(figure, ax1, graph_trigger_jiahui, "C12", legend="Jiahui", ylabel="ISS/MC", col="grey")
    ax1.plot(kNucleiBinsRebin_center, eff_cor_trigger_jw(np.log(kNucleiBinsRebin_center)), "-", color="grey")
    ax2.plot(kNucleiBinsRebin_center, fit_trigger_eff/eff_cor_trigger_jw(np.log(kNucleiBinsRebin_center)), "-", color="red", label="ratio: fit")
    ax2.plot(graph_trigger_jiahui.xvalues, graph_trigger.yvalues[:-1]/graph_trigger_jiahui.yvalues, ".", markersize=20, label="ratio: data", color="black")

    print(graph_trigger_jiahui.xvalues)
    print(graph_trigger.xvalues)
    ...
    ax2.text(0.05, 0.95, "Difference within 0.05%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="red", weight='bold')
    set_plot_defaultstyle(ax1)
    set_plot_defaultstyle(ax2)
    ax2.grid(linewidth=1)
    plt.subplots_adjust(hspace=.0)
    figure.subplots_adjust(left= 0.14, right=0.96, bottom=0.1, top=0.95)  
    savefig_tofile(figure, args.resultdir, f"hist_compare_jw_trigger_effcor", show=True)

    #trigger ISS
    fig, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.55, 0.45]}, figsize=(18, 13))
    fig.subplots_adjust(left= 0.13, right=0.95, bottom=0.1, top=0.95) 
    plot_graph(fig, ax1, dictgraph_eff_iss['Carbon']["trigger"], color=color_iso['C12'], label=f"this C", style="EP", xlog=False, ylog=False, markersize=20)
    plot_graph(fig, ax1, graph_trigger_jiahui_iss,  label=f"Jiahui C", style="EP", markersize=21, color=color_iso['C12'], markerfacecolor='none')
    ax1.text(0.05, 0.97, "ISS: Trigger", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    ax2.plot(graph_trigger_jiahui_iss.xvalues, dictgraph_eff_iss['Carbon']["trigger"].yvalues[:-1]/graph_trigger_jiahui_iss.yvalues, ".", markersize=20, label="ratio: data", color="black")
    ...
    set_plot_defaultstyle(ax1)
    set_plot_defaultstyle(ax2)
    ax2.text(0.05, 0.95, "Difference within 0.5%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="red", weight='bold')
    plt.grid()
    
    savefig_tofile(fig, args.resultdir, f"hist_compare_jw_trigger_eff_iss", show=True)
    #trigger MC

    fig, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.55, 0.45]}, figsize=(18, 13))
    fig.subplots_adjust(left= 0.13, right=0.95, bottom=0.1, top=0.95) 
    plot_graph(fig, ax1, dictgraph_eff_mc['Carbon']['C12']["trigger"], color='tab:blue', label=f"this C12", style="EP", xlog=False, ylog=False, markersize=20)
    plot_graph(fig, ax1, graph_trigger_jiahui_mc,  label=f"Jiahui C12", style="EP", markersize=21, color='tab:blue', markerfacecolor='none')
    ax1.text(0.05, 0.97, "MC: Trigger", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    ax2.plot(graph_trigger_jiahui_mc.xvalues, dictgraph_eff_mc['Carbon']['C12']["trigger"].yvalues[:-1]/graph_trigger_jiahui_mc.yvalues, ".", markersize=20, label="ratio: data", color="black")
    #plot_graph_eff(fig, ax1, graph_trigger_jiahui_mc, "C12", legend="Jiahui MC C12", col="magenta")
    ax1.set_ylim([0.99, 1.002])
    ax1.set_ylabel("Efficiency")
    ax1.legend(loc='lower left', fontsize=FONTSIZE)
    ax1.set_xscale('log')
    ...
    set_plot_defaultstyle(ax1)
    set_plot_defaultstyle(ax2)
    savefig_tofile(fig, args.resultdir, f"hist_compare_jw_trigger_eff_mc", show=True)

        
    dict_graph_trigger = dict()
    graph_trigger.add_to_file(dict_graph_trigger, "graph_trigger_effcor")
    spline_trigger_effcor = UnivariateSpline(np.log(graph_trigger.xvalues), graph_trigger.yvalues, k=3, s=100)
    np.savez(os.path.join(args.savedatadir, f"graph_trigger_effcor.npz"), **dict_graph_trigger)
    np.savez(os.path.join(args.savedatadir, f"graph_trigger_eff.npz"), **df_graph_effissmc)
    save_spline_to_file(spline_trigger_effcor, args.savedatadir, "spline_trigger_effcor.pickle")
