...
from tools.graphs import MGraph, slice_graph, concatenate_graphs, plot_graph, scale_graph
...
def plot_comparison_twohist(figure=None, ax1=None, ax2=None, histA=None, histB=None, ratio=None, ratioerr=None, xlog=True, ylog=False, isotope=None, xlabel=None, ylabel=None, figname=None, save=False, cutname=None):
    ...
    plot1dhist(figure, ax2, xbinning=xbinning.edges[1:-1], counts=ratio, err=ratioerr,  label_x=xlabel, label_y="Data/MC", legend=None,  col="black", setlogx=True, setlogy=False, setscilabelx=False,  setscilabely=False)
    plt.subplots_adjust(hspace=.0)                                                               
    ax1.legend()
    ax2.sharex(ax1)
    if (save):
        savefig_tofile(figure, "plots/sectrk", f"hist_{figname}_rig_passtotalevent", show=False)
...
cutplotlim = {"background": {"He": [0.96, 1.02], "C": [0.89, 1.01], "Be": [0.85, 1.01]}}
effcorplotlim = {"background": {"He": [0.98, 1.005], "C": [0.94, 1.0], "Be": [0.9, 1.0]}}
...
def get_efficiency(nuclei, filename, datatype, variable):
    ...
    with uproot.open(filename) as rootfile:
        for i, cut in enumerate(studyeff):
            ...
            ax2.set_ylim(cutplotlim[cut][nuclei])                                                                     
...
def main():
    ...
    parser.add_argument("--resultdir", default="plots/sectrk", help="Directory to store plots and result files in.")
    ...
    parser.add_argument("--savedatadir", default="plots/sectrk", help="dataname for the output file and plots.")
    
    args = parser.parse_args()
    os.makedirs(args.resultdir, exist_ok=True)
    nucleus = ["He", "C", "Be"]
    nuclei_alias = {"He": "He", "C": "Carbon", "Be": "Be"}
    ...
    pd_jiahui_bg = pd.read_csv("/home/manbing/Documents/Data/jiahui/efficiencies/bkg_cut_eff.csv",  sep='\s+', header=0)
    graph_bg_jiahui = dict()
    graph_bg_jiahui_mc = dict()
    graph_bg_jiahui_iss = dict()
    df_graph_effissmc = dict()
    for nuclei in nucleus:
        if nuclei != "Be":
            graph_bg_jiahui[nuclei] = MGraph(xvalues=pd_jiahui_bg["bin_center[GV]"], yvalues=pd_jiahui_bg[f"{nuclei}_dt"]/pd_jiahui_bg[f"{nuclei}_MC"],
                                             yerrs=get_ratio_err(pd_jiahui_bg[f"{nuclei}_dt"], pd_jiahui_bg[f"{nuclei}_MC"], pd_jiahui_bg[f"{nuclei}_dt_err"], pd_jiahui_bg[f"{nuclei}_MC_err"]))

            graph_bg_jiahui_iss[nuclei] = MGraph(xvalues=pd_jiahui_bg["bin_center[GV]"], yvalues=pd_jiahui_bg[f"{nuclei}_dt"], yerrs=pd_jiahui_bg[f"{nuclei}_dt_err"])
            graph_bg_jiahui_mc[nuclei] = MGraph(xvalues=pd_jiahui_bg["bin_center[GV]"], yvalues=pd_jiahui_bg[f"{nuclei}_MC"], yerrs=pd_jiahui_bg[f"{nuclei}_MC_err"])
    ...
    for nuclei in nucleus:
        ...
        for i, isotope in enumerate(isotopes[nuclei]):        
            ...
            for cut in studyeff:
                ...
                dict_eff_cor[nuclei][isotope][cut].add_to_file(df_graph_effissmc, f'grapheff_datamc_ratio_{isotope}_{cut}')
        ...
        for cut in studyeff:  
            figure, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.6, 0.4]}, figsize=(20, 15))
            plot_graph_eff(figure, ax1, dictgraph_eff_iss[nuclei][cut], "ISS", legend=f"{nuclei} ISS")
            ...
            ax1.set_ylim(cutplotlim[cut][nuclei])
            ax2.set_ylim(effcorplotlim[cut][nuclei])
            ...
            ax1.legend(loc='lower left', fontsize=FONTSIZE-2)
    ...
    np.savez(os.path.join(args.savedatadir, f"graph_sedtrk_eff.npz"), **df_graph_effissmc)

    #plot corrections comparison
    print(dict_eff_cor)
    col_nuclei = {"He": "tab:blue", "C": "tab:orange", "Be":"tab:green"}
    for nuclei in nucleus:
        fig, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.5, 0.5]}, figsize=(19, 15))
        ...
        for i, iso in enumerate(isotopes[nuclei]):
            plot_graph(fig, ax1, dict_eff_cor[nuclei][iso]["background"], color=color_iso[iso], label=f"this", style="EP", xlog=False, ylog=False, markersize=20)
        if nuclei != 'Be':
            plot_graph(fig, ax1, graph_bg_jiahui[nuclei], color="grey", label=f"Jiahui", style="EP", xlog=False, ylog=False, markersize=20)
            ax2.plot(graph_bg_jiahui[nuclei].xvalues, dict_eff_cor[nuclei][iso]["background"].yvalues[2:]/graph_bg_jiahui[nuclei].yvalues, ".", markersize=20, color="black", label="ratio: data")
            
        ax1.set_ylim(effcorplotlim["background"][nuclei])
        ax1.set_ylabel("Data/MC")
        ax1.legend(loc="lower left", fontsize=FONTSIZE)
        ax1.text(0.05, 0.88, f"{nuclei_alias[nuclei]}", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color=col_nuclei[nuclei], fontweight="bold")
        ax1.text(0.05, 0.97, f"Background Reduction Cut", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
        ax1.get_yticklabels()[0].set_visible(False)
        ...
        ax2.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
        ax1.sharex(ax2) 
        ax2.set_ylabel("this/Jiahui")
        ax2.set_xscale("log")
        ax2.set_xlabel("Rigidity (GV)")
        plt.subplots_adjust(hspace=.0)
        ax2.grid()
        if nuclei == "He":
            ax2.set_ylim([0.999, 1.002])
            ax2.text(0.05, 0.97, "Difference within 0.1%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="red", weight='bold')
        else:
            ax2.set_ylim([0.999, 1.003])
            ax2.text(0.05, 0.97, "Difference within 0.2%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="red", weight='bold')
        ax2.legend(loc='lower right' if nuclei == "He" else 'lower left', fontsize=FONTSIZE)
        savefig_tofile(fig, args.resultdir, f"compare_{nuclei}_background_cut_effcor", show=True)

    #plot mc comparison
    iso_labels = {"C12": "$\mathrm{^{12}C}$", "He4": "$\mathrm{^{4}He}$", "Be7": "$\mathrm{^{7}Be}$", "Be9": "$\mathrm{^{9}Be}$", "Be10": "$\mathrm{^{10}Be}$"}
    
    iso_ratio_guess = {"Be7": 0.6, "Be9": 0.3, "Be10": 0.1, "B10": 0.5, "B11":0.5, "C12":1.0, "O16": 1.0, "N14": 0.5, "N15": 0.5}
    graph_bkg_be_average = MGraph(dictgraph_eff_mc["Be"]["Be7"]["background"].xvalues, np.zeros(len(dictgraph_eff_mc["Be"]["Be7"]["background"].xvalues)), np.zeros(len(dictgraph_eff_mc["Be"]["Be7"]["background"].xvalues)))
    for i, iso in enumerate(isotopes["Be"]):
        graph_bkg_be_average =  graph_bkg_be_average + scale_graph(dictgraph_eff_mc["Be"][iso]["background"], iso_ratio_guess[iso])   

    #for nuclei in nucleus:    
        #for i, iso in enumerate(isotopes[nuclei]):
        #   iso_symbol = iso_labels[iso]
    fig, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.55, 0.45]}, figsize=(20, 15))
    fig.subplots_adjust(left= 0.13, right=0.95, bottom=0.1, top=0.95)
    plot_graph(fig, ax1, dictgraph_eff_mc["He"]["He4"]["background"], color=color_iso["He4"], label=r"$\mathrm{^{4}He}$", style="EP", xlog=False, ylog=False, markersize=20)
    #plot_graph(figure, ax1, graph_bkg_be_average,  label="Mix MC Be", style="EP", markersize=21, color="red")
    plot_graph(fig, ax1, dictgraph_eff_mc["C"]["C12"]["background"], color=color_iso["C12"], label=r"$\mathrm{^{12}C}$", style="EP", xlog=False, ylog=False, markersize=20)
    plot_graph(fig, ax1, graph_bg_jiahui_mc['He'],  label=r"$\mathrm{Jiahui}$ $\mathrm{^{4}{He}}$", style="EP", markersize=22, color=color_iso['He4'], markerfacecolor='none')
    plot_graph(fig, ax1, graph_bg_jiahui_mc['C'],  label=r"$\mathrm{Jiahui}$ $\mathrm{^{12}{C}}$", style="EP", markersize=22, color=color_iso['C12'], markerfacecolor='none')

    ax2.plot(graph_bg_jiahui_mc["He"].xvalues, dictgraph_eff_mc["He"]["He4"]["background"].yvalues[2:]/graph_bg_jiahui_mc["He"].yvalues, ".", markersize=20, color=color_iso['He4'])
    ax2.plot(graph_bg_jiahui_mc["C"].xvalues, dictgraph_eff_mc["C"]["C12"]["background"].yvalues[2:]/graph_bg_jiahui_mc["C"].yvalues, ".", markersize=20, color=color_iso['C12'])

    #ax1.errorbar(graph_bg_jiahui_mc[nuclei].xvalues, graph_bg_jiahui_mc[nuclei].yvalues, graph_bg_jiahui_mc[nuclei].yerrs, fmt='o', markersize=16, color=color_iso[iso], markerfacecolor='none') 
    ax1.text(0.06, 0.98, "MC: Background Reduction Cut", fontsize=FONTSIZE_BIG, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    ax1.set_ylim([0.92, 1.02])
    ax1.legend(loc="lower left", fontsize=FONTSIZE)
    ax1.set_xlim([1.8, 1000])
    ax1.set_xscale("log")
    ax1.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ...
    ax2.set_xscale("log")
    ax2.set_xlim([1.8, 1000])
    ax2.set_xticks([2, 5, 10, 30,  100,  300, 800])
    ax2.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ax1.sharex(ax2) 
    ax2.set_ylabel("this/Jiahui")
    ...
    ax2.text(0.05, 0.97, "Difference within 0.1%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="red", weight='bold')
    ax2.legend(loc='lower right' if nuclei == "He" else 'lower left', fontsize=FONTSIZE)
    savefig_tofile(fig, args.resultdir, f"compare_sedtrkcut_eff_mc", show=True)
    ...
    ax1.set_xscale("log")
    ax1.set_xlim([1.8, 1000])
    ax1.set_xticks([2, 5, 10, 30,  100,  300, 800])
    ax1.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    ax1.sharex(ax2) 
    ax1.set_ylabel("this/Jiahui")
    plt.subplots_adjust(hspace=.0)
    ax1.grid()
    ax1.legend(loc='lower right' if nuclei == "He" else 'lower left', fontsize=FONTSIZE)
    savefig_tofile(fig, args.resultdir, f"compare_sedtrkcut_eff_mc_2", show=True)

    #plot iss data comparison
    fig, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios':[0.55, 0.45]}, figsize=(20, 15))
    fig.subplots_adjust(left= 0.13, right=0.95, bottom=0.1, top=0.95) 
    plot_graph(fig, ax1, dictgraph_eff_iss['He']["background"], color=color_iso['He4'], label=f"He", style="EP", xlog=False, ylog=False, markersize=20)
    #plot_graph(fig, ax1, dictgraph_eff_iss['Be']["background"], color='red', label=f"Be", style="EP", xlog=False, ylog=False, markersize=20)
    plot_graph(fig, ax1, dictgraph_eff_iss['C']["background"], color=color_iso['C12'], label=f"C", style="EP", xlog=False, ylog=False, markersize=20)    
    plot_graph(figure, ax1, graph_bg_jiahui_iss['He'],  label=f"Jiahui He", style="EP", markersize=22, color=color_iso['He4'], markerfacecolor='none')
    plot_graph(figure, ax1, graph_bg_jiahui_iss['C'],  label=f"Jiahui C", style="EP", markersize=22, color=color_iso['C12'], markerfacecolor='none')

    ax2.plot(graph_bg_jiahui_iss["He"].xvalues, dictgraph_eff_iss["He"]["background"].yvalues[2:]/graph_bg_jiahui_iss["He"].yvalues, ".", markersize=20, color=color_iso['He4'])
    ax2.plot(graph_bg_jiahui_iss["C"].xvalues, dictgraph_eff_iss["C"]["background"].yvalues[2:]/graph_bg_jiahui_iss["C"].yvalues, ".", markersize=20, color=color_iso['C12'])

    #ax1.errorbar(graph_bg_jiahui_mc[nuclei].xvalues, graph_bg_jiahui_mc[nuclei].yvalues, graph_bg_jiahui_mc[nuclei].yerrs, fmt='o', markersize=16, color=color_iso[iso], markerfacecolor='none') 
    ax1.text(0.06, 0.98, "ISS: Background Reduction Cut", fontsize=FONTSIZE_BIG, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold")
    ...
    ax2.text(0.05, 0.97, "Difference within 0.1%", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax2.transAxes, color="red", weight='bold')
    ax2.legend(loc='lower right' if nuclei == "He" else 'lower left', fontsize=FONTSIZE)
    savefig_tofile(fig, args.resultdir, f"compare_sedtrkcut_eff_iss", show=True)
    plt.show()
