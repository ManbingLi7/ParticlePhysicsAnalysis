    ...
    for dec in detectors: 
        for i_iso, iso in enumerate(isotopes):
            for events in read_tree(filename_mc[i_iso], treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
                ...
                #hist_mcbetareso[dec][iso].fill(refEkin, beta_tof - refbeta, weights=mc_weight)
                #hist_mcbetareso_mix[dec].fill(refEkin, beta_tof - refbeta, weights=weight_mix)

                hist_mcbetareso[dec][iso].fill(true_beta_atRICH, beta_tof - true_beta_atRICH, weights=mc_weight)
                hist_mcbetareso_mix[dec].fill(true_beta_atRICH, beta_tof - true_beta_atRICH, weights=weight_mix)
...
def main():
    ...
    parser.add_argument("--filename_iss", default= "/home/manbing/Documents/Data/data_BeP8/rootfile/OxygenIss_P8_CIEBetaCor.root",  help="(e.g. results/ExampleAnalysisTree*.root)")
    ...
    parser.add_argument("--resultdir", default="/home/manbing/Documents/Data/data_BeP8/TofBeta", help="Directory to store plots and result files in.")
    ...
    #
    #args.filename_mc = [f"/home/manbing/Documents/Data/data_BeP8/rootfile/O16MC_B1236P8_CIEBetaCor_Tof2.root" for iso in isotopes]
    ...
    if usetunedRICH == True:
        np.savez(os.path.join(args.resultdir, f"{nuclei}_tofbetaResidual_ref{decref}_beta2Tuned_B1236P8mcweightV2_finebin.npz"), **result_resolution)
    else:
        np.savez(os.path.join(args.resultdir, f"{nuclei}_tofbetaResidual_ref{decref}_beta2_B1236P8mcweight_finebin.npz"), **result_resolution)
