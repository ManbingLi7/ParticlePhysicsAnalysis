...
import pickle
from tools.functions import calc_isoratio_witherr


detectors = ["Tof", 'NaF', 'Agl']
iso_ratio_guess = {"Li6": 0.5, "Li7": 0.5}

par_names = {"mean": 0.16, "sigma": 0.016, "fraccore":0.9, "sigma_ratio":1.27, "asy_factor":1.1}

scale_factor = {"Be7": 1, "Be9": 7./9., "Be10": 7.0/10.0, 'Li6': 1.0, 'Li7': 6.0/7.0}

mcpars_initlims = {'Tof': {'mean': (0.13, 0.18), 'sigma': (0.008, 0.03), 'fraccore': (0.7, 0.98), 'sigma_ratio':(1.1, 1.6), 'asy_factor':(1.1, 1.6)},
                   'NaF': {'mean': (0.13, 0.18), 'sigma': (0.008, 0.03), 'fraccore': (0.89, 0.98), 'sigma_ratio':(1.4, 1.8), 'asy_factor':(1.1, 1.25)},
                   'Agl': {'mean': (0.13, 0.18), 'sigma': (0.008, 0.03), 'fraccore': (0.8, 0.98), 'sigma_ratio':(1.3, 1.8), 'asy_factor':(0.9, 1.2)}}
...
elim_range_compare = {'mean': [0.08, 0.19],
                      'sigma':       [0.008, 0.024],
                      'sigma_ratio': [0.7, 2.5],
                      'asy_factor' : [0.8, 2.0],
                      'fraccore'   : [0.6, 1.0],
...
ylim_range_be7 = {'mean'  :     [0.163, 0.19], 'sigma' : [0.01, 0.03], 'sigma_ratio':[1.0, 2.2], 'asy_factor': [0.8, 1.8], 'fraccore':   [0.6, 1.1], "norm":       [0, 40]}
ylim_range_be = {'Li7':  {'mean': [0.163, 0.19], 'sigma' : [0.01, 0.03],   'sigma_ratio':[1.0, 2.2], 'asy_factor': [0.8, 1.6], 'fraccore':   [0.6, 1.1], "norm":       [0, 40]},
                 'Li6':  {'mean': [0.163, 0.19], 'sigma' : [0.01, 0.03],   'sigma_ratio':[1.0, 2.2], 'asy_factor': [0.8, 1.6], 'fraccore':   [0.6, 1.1], "norm":       [0, 40]}}

detectors_energyrange = {"Tof": [0.4, 0.9], "NaF": [0.8, 6.0], "Agl": [3.5, 11]} #Agl: 4.0, 10

fit_range_mass_nuclei = [0.08, 0.25]
...
def initial_guess(guess_initial, guess_pvalues, massfit, nuclei, detector, mufactorfile, sigfactorfile, isConstraint):
    ...
    if isConstraint:
        for iso in ISOTOPES[nuclei][0:-1]:
            isonum = NUCLEI_NUMBER[iso]
...
def main():
    ...
    #parser.add_argument("--filedata", default="/home/manbing/Documents/Data/data_LiP8/Hist2D/LiISS_Ekin_finebin_P8.npz", help="file of data (mass vs energy 2d histogram) to be analysis")
    ...
    parser.add_argument("--dirmc", default="/home/manbing/Documents/lithiumanalysis/scripts/plots/LiP8/LiMC_hist_MassHist_B1308_FineNewBin_ToFShiftCutoff_CutToFEdge/",  help="Path to root file to read tree from") 
    #parser.add_argument("--filedata", default="/home/manbing/Documents/Data/data_LiP8/Hist2D/LiISS_Ekin_P8GBL_FineBin_NoTofEdge.npz", help="file of data (mass vs energy 2d histogram) to be analysis")    
    #parser.add_argument("--plotdir", default="plots/LiP8/fitDataB1308_rebin", help="Directory to store plots and result files in.")
    parser.add_argument("--plotdir", default="plots/LiTest/fitDataB1308_finebin_ToFShiftCutoff_CutToFEdge_16May_Final", help="Directory to store plots and result files in.")    
    ...
    p_pars_mc1308 = np.load(os.path.join(args.dirmc, 'pvalues_mc7_iter1.npz'), allow_pickle=True)
    ...
    dictpar = {}
    print(p_pars_mc1308['Tof'].item())
    print(p_pars_mc1236['Tof'].item())
    ...
    print('mufactorfile:', mufactorfile.items())
    ...
    guesserr_dec = {'Tof': '/home/manbing/Documents/Data/data_BeP8/FitParsRange/splines_pars_uncertaintymax_BetaRig.pkl',
                    'NaF': None, 
    ...
    if args.isconstraint is False:  
        graph_ratio = {dec: {} for dec in detectors}
        for dec in detectors:
            for OpIso in OptimizedDataSample:
                num_iso = len(massfit[OpIso][dec].isotopes)    
                num_common_pars = massfit[OpIso][dec].numpvalues + 2 * 3  * (num_iso -1)
                nbins = massfit[OpIso][dec].num_energybin
            
                ratio_results = calc_isoratio_witherr(massfit[OpIso][dec].x_fit_energy, fit_parameters[OpIso][dec][num_common_pars+nbins:], fit_parameters[OpIso][dec][num_common_pars: num_common_pars+nbins])    
                yratio, yratio_err = unumpy.nominal_values(ratio_results), unumpy.std_devs(ratio_results) 
                graph_ratio[dec][OpIso] = MGraph(massfit[OpIso][dec].x_fit_energy, yratio, yratio_err)
                graph_ratio[dec][OpIso].add_to_file(dict_graph_counts, f'graph_ratio_{dec}Opt{OpIso}')
    
    counts_iso1 = {dec: {} for dec in detectors}
    iso0 = ISOTOPES[nuclei][0]
    iso1 = ISOTOPES[nuclei][1]
    if args.isconstraint: 
        for dec in detectors:
            for OpIso in OptimizedDataSample:
                counts_iso1[dec][OpIso] = massfit[OpIso][dec].get_data_infitrange().sum(axis=1) - fit_norm[dec][OpIso][6] 
                yerrli7 = graph_counts_iso[dec][OpIso][iso0].yerrs 
                graph_counts_iso[dec][OpIso][iso1] =  MGraph(massfit[OpIso][dec].x_fit_energy, counts_iso1[dec][OpIso], yerrli7)     
                graph_counts_iso[dec][OpIso][iso1].add_to_file(dict_graph_counts, f'graph_counts_{dec}Opt{OpIso}_Li7')
