...
detectors_energyrange = {"Tof": [0.45, 1.3], "NaF": [1.0, 5.0], "Agl": [4.0, 12]} #Agl: 4.0, 10

fit_range_mass_nuclei = [0.07, 0.25]
...
def update_guess_binbybinfit_initia_bybinnum(massfits, graph_pars, isotopes):
    guess_binbybin = {}
    xfitenergy = massfits.x_fit_energy
    
    if (len(xfitenergy) != len(graph_pars['mean'].xvalues) or abs(xfitenergy[0] - graph_pars['mean'].xvalues[0]) > 0.01):
        print('energy range not consistent')
        
    else:
        counts = massfits.get_data_infitrange()
        for name, value in par_names.items():
            print(name, 'relative error:',  abs(graph_pars[name].yerrs[0])/graph_pars[name].yvalues[0])
            for ibin in range(massfits.num_energybin):
                err_random = np.random.uniform(-abs(graph_pars[name].yerrs[ibin]), abs(graph_pars[name].yerrs[ibin]))
                guess_binbybin[f'{name}_{ibin}'] = graph_pars[name].yvalues[ibin] + err_random
        for iso in isotopes:
            isonum = NUCLEI_NUMBER[iso]
            for ibin in range(massfits.num_energybin):
                guess_binbybin[f'n{isonum}_{ibin}'] = counts[ibin].sum() * iso_ratio_guess[iso]
    print(guess_binbybin)
    return guess_binbybin


def update_guess_binbybinfit_initial_byvalue(massfits, graph_pars, isotopes):
    guess_binbybin = {}
    xfitenergy = massfits.x_fit_energy
    ...
    for name, value in par_names.items():
        print(name, 'relative error:',  abs(graph_pars[name].yerrs[0])/graph_pars[name].yvalues[0])
        for ibin in range(massfits.num_energybin):
            graphindex = graph_pars[name].get_index(xfitenergy[ibin])
            err_random = np.random.uniform(-abs(graph_pars[name].yerrs[graphindex]), abs(graph_pars[name].yerrs[graphindex]))
            guess_binbybin[f'{name}_{ibin}'] = graph_pars[name].yvalues[graphindex] + err_random
    for iso in isotopes:
        isonum = NUCLEI_NUMBER[iso]
        for ibin in range(massfits.num_energybin):
            guess_binbybin[f'n{isonum}_{ibin}'] = counts[ibin].sum() * iso_ratio_guess[iso]
...
def main():
    ...
    #parser.add_argument("--filedata", default="/home/manbing/Documents/Data/data_LiP8/Hist2D/LiISS_Ekin_Lithium_FromCarbon.npz", help="file of data (mass vs energy 2d histogram) to be analysis")
    parser.add_argument("--filedata", default="/home/manbing/Documents/Data/data_LiP8/Hist2D/LiISS_Ekin_newbin_P8_cutToFEdge.npz", help="file of data (mass vs energy 2d histogram) to be analysis")    
    parser.add_argument("--plotdir", default="plots/LiP8/fitData_finebin_binbybin_May", help="Directory to store plots and result files in.")
    parser.add_argument("--nuclei", default="Li", help="Directory to store plots and result files in.")
    parser.add_argument("--isconstraint", default=False, help="choose if constraint the total N in fitting data")
    ...
    parser.add_argument("--isfitsimu", default=False, type=bool, help="choose if is_nonlinearconstraint")
    parser.add_argument("--niter", default=50, type=int, help="choose if is_nonlinearconstraint")
    ...
    with np.load(filenames) as massfile:
        for dec in detectors:
            for OpIso in OptimizedDataSample:
                hist2d_mass_energy[dec][OpIso] = Histogram.from_file(massfile, f"{nuclei}_{dec}Opt{OpIso}_mass_ciemat")
    ...
    massfit = {iiter: {} for iiter in range(args.niter)}
    fit_parameters = {iiter: {} for iiter in range(args.niter)}
    dict_fit_parameters = {iiter: {dec: {} for dec in detectors} for iiter in range(args.niter)}
    isscovariance = {iiter: {dec: {} for dec in detectors} for iiter in range(args.niter)}
    ##############################################################################################
    ...
    fixed_pvalues = {'Tof': [ 'mean', 'sigma', 'asy_factor', 'sigma_ratio', 'fraccore'],
                     'NaF': [ 'mean', 'sigma', 'asy_factor', 'sigma_ratio', 'fraccore'],
                     'Agl': [ 'mean', 'sigma', 'asy_factor', 'sigma_ratio', 'fraccore']}
    #fixed_pvalues = {'Tof': [],
    #                 'NaF': [],
    #                 'Agl': []}
    ...
    filedir_fitdatapars = '/home/manbing/Documents/lithiumanalysis/scripts/plots/LiP8/fitDataB1308_finebin_ShiftToF_CutToFEdge_11May_NotConstraintNTotal'
    graph_fitpars = {dec: {} for dec in detectors}
    for par in par_names.keys():
        npzfile = np.load(os.path.join(filedir_fitdatapars, f'graph_fitpar_{par}_Li7.npz'))
        for dec in detectors:
            graph_fitpars[dec][par] = MGraph.from_file(npzfile, f'graph_{dec}_{par}')
            print(graph_fitpars[dec][par])
    guess_initial = {dec: {} for dec in detectors}
    OpIso = 'Li6'

    for iiter in range(args.niter):
        for dec in detectors:
            massfit[iiter][dec] = MassFunctionFit[dec](nuclei, isotopes, hist2d_mass_energy[dec][OpIso], detectors_energyrange[dec], fit_range_mass_nuclei, dec, args.isconstraint)
            guess_initial[dec] = update_guess_binbybinfit_initial_byvalue(massfit[iiter][dec], graph_fitpars[dec], isotopes)
            ...
            fit_parameters[iiter][dec], dict_fit_parameters[iiter][dec], isscovariance[iiter][dec] = massfit[iiter][dec].perform_fit(guess_initial[dec], guesserr='/home/manbing/Documents/Data/data_BeP8/FitParsRange/splines_pars_uncertaintymax_BetaRig.pkl',  fit_simultaneous=False,  fixed_pars = fixed_pars_beiss[dec], verbose=True, parlim=ErrorBandLim[dec], fitFreeP=args.isFreeP)
            ...
            for name in guess_initial[dec].keys():
                print(name, 'guess:', guess_initial[dec][name], ' ', 'fitResult:', dict_fit_parameters[iiter][dec][name]['value'])
                
            massfit[iiter][dec].draw_fit_results(fit_parameters[iiter][dec], dict_fit_parameters[iiter][dec], args.plotdir, fit_simultaneous=False)

   

        
    graph_counts_iso = {iiter: {dec: {} for dec in detectors}  for iiter in range(args.niter)}
    ...
    fit_norm =  {iiter: {}  for iiter in range(args.niter)}
    fit_norm_err =  {iiter: {}  for iiter in range(args.niter)}

    
    for iiter in range(args.niter):
        for dec in detectors:
            fit_norm[iiter][dec] = {isonum: np.array([dict_fit_parameters[iiter][dec][f"n{isonum}_{ibin}"]['value'] for ibin in range(massfit[iiter][dec].num_energybin)]) for isonum in isotopes_atom_num}
            fit_norm_err[iiter][dec] = {isonum: np.array([dict_fit_parameters[iiter][dec][f"n{isonum}_{ibin}"]['error'] for ibin in range(massfit[iiter][dec].num_energybin)]) for isonum in isotopes_atom_num}
            for iso, isonum in enumerate(isotopes_atom_num):
                isoname = ISOTOPES[args.nuclei][iso]
                graph_counts_iso[iiter][dec][isoname] = MGraph(massfit[iiter][dec].x_fit_energy, fit_norm[iiter][dec][isonum], fit_norm_err[iiter][dec][isonum])
                graph_counts_iso[iiter][dec][isoname].add_to_file(dict_graph_counts, f'graph_counts_{dec}_{ISOTOPES[args.nuclei][iso]}_{iiter}')

    graph_counts_iso_max = {iso: {} for iso in ISOTOPES[args.nuclei]}
    graph_counts_iso_min = {iso: {} for iso in ISOTOPES[args.nuclei]}
    graph_counts_iso_stderr = {iso: {} for iso in ISOTOPES[args.nuclei]}
    for iso, isonum in enumerate(isotopes_atom_num):
        isoname = ISOTOPES[args.nuclei][iso]
        for dec in detectors:
            isodata = np.vstack([fit_norm[iiter][dec][isonum] for iiter in range(args.niter)])
            graph_counts_iso_max[isoname][dec] = MGraph(massfit[0][dec].x_fit_energy, np.max(isodata, axis=0), np.zeros_like(massfit[0][dec].x_fit_energy))
            graph_counts_iso_min[isoname][dec] = MGraph(massfit[0][dec].x_fit_energy, np.min(isodata, axis=0), np.zeros_like(massfit[0][dec].x_fit_energy))
            
            data_without_extremes = isodata.copy()
            max_index = np.argmax(isodata, axis=0)
            min_index = np.argmax(isodata, axis=0)
            for i in range(isodata.shape[1]):
                data_without_extremes[max_index[i], i] = np.nan
                data_without_extremes[min_index[i], i] = np.nan 
            graph_counts_iso_stderr[isoname][dec] = MGraph(massfit[0][dec].x_fit_energy, np.nanstd(data_without_extremes, axis=0), np.zeros_like(massfit[0][dec].x_fit_energy))
            graph_counts_iso_min[isoname][dec].add_to_file(dict_graph_counts, f'graph_counts_min_{dec}_{isoname}')
            graph_counts_iso_max[isoname][dec].add_to_file(dict_graph_counts, f'graph_counts_max_{dec}_{isoname}')
            graph_counts_iso_stderr[isoname][dec].add_to_file(dict_graph_counts, f'graph_counts_stderr_{dec}_{isoname}')
            
    counts_be10 = {dec: {} for dec in detectors}
    if args.isconstraint: 
        for iiter in OptimizedDataSample:
            for dec in detectors:
                counts_li7[iiter][dec] = massfit[iiter][dec].get_data_infitrange().sum(axis=1) - fit_norm[iiter][dec][6]
                yerrli7 = np.sqrt(graph_counts_iso[iiter][dec]["Li6"].yerrs ** 2)
                graph_counts_iso[iiter][dec]["Li7"] = MGraph(massfit[iiter][dec].x_fit_energy, counts_li7[iiter][dec], yerrli7)     
                graph_counts_iso[iiter][dec]["Li7"].add_to_file(dict_graph_counts, f'graph_counts_{dec}_Li7_{iiter}')
