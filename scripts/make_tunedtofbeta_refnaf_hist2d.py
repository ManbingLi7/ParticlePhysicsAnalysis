...
from tools.utilities import get_spline_from_file
...
def handle_file(arg):    
    ...
    df_spline_betakor =  np.load('/home/manbing/Documents/Data/data_BeP8/spline_tofbeta_sigmakorr.npz', allow_pickle=True)  
    spline_tofbeta_sigmakorr  = df_spline_betakor['sigmakorr_NaF']
    ...
    for dec in detectors: 
        for i_iso, iso in enumerate(isotopes):
            for events in read_tree(filename_mc[i_iso], treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
                ...
                beta_tof_tuned = GetMCTofTunedBeta_WithSpline(events, df_spline_betakor[f'sigmakorr_{decref}'], iso)
                ...
                hist_mcbetareso[dec][iso].fill(ekin_rich, beta_tof_tuned - beta_rich, weights=mc_weight)
                hist_mcbetareso_mix[dec].fill(ekin_rich, beta_tof_tuned - beta_rich, weights=weight_mix)
...
def main():
    ...
    #args.filename_mc = [f"/home/manbing/Documents/Data/data_BeP8/rootfile/C12MC_B1236P8_CIEBetaCor.root" for iso in isotopes]
    ...
    np.savez(os.path.join(args.resultdir, f"{nuclei}_tofbetaResidualTuned_ref{decref}_beta2_B1236P8mcweight.npz"), **result_resolution)
