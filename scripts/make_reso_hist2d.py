...
binning_beta_residual = {"Tof": make_lin_binning(-0.07, 0.04, 200), "NaF": make_lin_binning(-0.03, 0.01, 300), "Agl": make_lin_binning(-0.006, 0.003, 300)}
...
def handle_file(arg):    
    ...
    hist_issEkin = {}
    hist_beta_reso_dict = dict()
    hist_beta_residual_vsbeta = {dec: {} for dec in detectors}
    hist_beta_residual_vsEkin = {dec: {} for dec in detectors}
    hist_beta_refRGBL_vsEkin  = {dec: {} for dec in detectors}
    hist_beta_refRCho_vsEkin = {dec: {} for dec in detectors}
    ...
    for dec in detectors:
        ...
        for iso in isotopes:
            ...
            hist_beta_residual_vsEkin[dec][iso] = WeightedHistogram(xbinning['Ekin'][dec], binning_beta_residual[dec], labels=[r"Ekin/n(GeV/n)", r"$\mathrm{\beta_{m}-\beta{t}}$"])
            hist_beta_refRGBL_vsEkin[dec][iso] = WeightedHistogram(xbinning['Ekin'][dec], binning_beta_residual[dec], labels=[r"Ekin/n(GeV/n)", r"$\mathrm{\beta_{m}-\beta{GBL}}$"])
            hist_beta_refRCho_vsEkin[dec][iso] = WeightedHistogram(xbinning['Ekin'][dec], binning_beta_residual[dec], labels=[r"Ekin/n(GeV/n)", r"$\mathrm{\beta_{m}-\beta{R\_choutko}}$"])
    ...
    for dec in detectors: 
        for events in read_tree(filename_iss, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
            ...
            rigidityRCho = ak.to_numpy((events.tk_rigidity1)[:, 0, 2, 1])
            rigidityRGBL = ak.to_numpy((events.tk_rigidity1)[:, 1, 2, 1])
    ...
    for dec in detectors: 
        for i_iso, iso in enumerate(isotopes):
            for events in read_tree(filename_mc[i_iso], treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
                ...
                hist_beta_resolution[dec][iso].fill(true_ekin, beta/true_beta - 1, weights=mc_weight)
                hist_beta_residual_vsEkin[dec][iso].fill(ekin_new, beta-true_beta, weights=weight_one)
                hist_beta_refRGBL_vsEkin[dec][iso].fill(ekin_new, beta-beta_RGBL, weights=weight_one)
                hist_beta_refRCho_vsEkin[dec][iso].fill(ekin_new, beta-beta_RCho, weights=weight_one)

            hist_beta_resolution[dec][iso].add_to_file(hist_beta_reso_dict, f'beta_resolution_{dec}_{iso}')
            hist_beta_residual_vsEkin[dec][iso].add_to_file(hist_beta_reso_dict, f'hist2d_beta_refTrue_{dec}_{iso}')
            hist_beta_refRGBL_vsEkin[dec][iso].add_to_file(hist_beta_reso_dict, f'hist2d_beta_refRGBL_{dec}_{iso}')
            hist_beta_refRCho_vsEkin[dec][iso].add_to_file(hist_beta_reso_dict, f'hist2d_beta_refRCho_{dec}_{iso}')
            
    np.savez(os.path.join(resultdir, f"betaresidual_{rank}.npz"), **hist_beta_reso_dict)
...
def main():
    ...
    for rank in range(args.nprocesses):
        filename_response =  os.path.join(args.resultdir, f"betaresidual_{rank}.npz")  
        with np.load(filename_response) as reso_file:
            for dec in detectors:
                ...
                for iso in isotopes:
                    ...
                    hist_beta_residual_vsEkin[dec][iso] += WeightedHistogram.from_file(reso_file, f"hist2d_beta_refTrue_{dec}_{iso}")
                    hist_beta_refRGBL_vsEkin[dec][iso] += WeightedHistogram.from_file(reso_file, f"hist2d_beta_refRGBL_{dec}_{iso}")
                    hist_beta_refRCho_vsEkin[dec][iso] += WeightedHistogram.from_file(reso_file, f"hist2d_beta_refRCho_{dec}_{iso}")
    ...
    for dec in detectors:
        ...
        for iso in isotopes: 
            ...
            hist_beta_residual_vsEkin[dec][iso].add_to_file(result_resolution, f"hist2d_beta_refTrue_{dec}_{iso}")
            hist_beta_refRGBL_vsEkin[dec][iso].add_to_file(result_resolution, f"hist2d_beta_refRGBL_{dec}_{iso}")
            hist_beta_refRCho_vsEkin[dec][iso].add_to_file(result_resolution, f"hist2d_beta_refRCho_{dec}_{iso}")

    np.savez(os.path.join(args.resultdir, f"BeMCB1236_beta_residual_v2.npz"), **result_resolution)
