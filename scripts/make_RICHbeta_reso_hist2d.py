...
binning_inversebeta_residual = {"Tof": make_lin_binning(-0.07, 0.15, 300), "NaF": make_lin_binning(-0.008, 0.008, 70), "Agl": make_lin_binning(-0.003, 0.003, 100)}
...
def handle_file(arg):    
    ...
    for dec in detectors:
        hist_issbetareso[dec] = WeightedHistogram(xekinbinning, binning_inversebeta_residual[dec], labels=['Ekin/n (GeV/n)', r"$\mathrm{1/\beta}$"])
        hist_mcbetareso_mix[dec] = WeightedHistogram(xekinbinning, binning_inversebeta_residual[dec], labels=['Ekin/n (GeV/n)', r"$\mathrm{1/\beta}$"])
        for iso in isotopes:
            hist_mcbetareso[dec][iso] = WeightedHistogram(xekinbinning, binning_inversebeta_residual[dec], labels=['Ekin/n (GeV/n)', r"$\mathrm{1/\beta}$"])
    ...
    for dec in detectors: 
        for i_iso, iso in enumerate(isotopes):
            for events in read_tree(filename_mc[i_iso], treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
                ...
                true_momentum_atDec = ak.to_numpy(events["mevmom1"][:, 17])
                true_rigidity_atDec = ak.to_numpy(events["mevmom1"][:, 17]/NUCLEI_CHARGE[nuclei])
                true_beta_atDec = calc_beta(true_rigidity_atDec, ISOTOPES_MASS[iso], ISOTOPES_CHARGE[iso])
                true_ekin_atDec = calc_ekin_from_rigidity_iso(true_rigidity_atDec, iso)
                
                beta_tof = ak.to_numpy(events['tof_betahmc'])
                beta_rich = ak.to_numpy(events['rich_beta_cor'])
                beta_rich_tuned = GetMCRICHTunedBeta_WithHighR(events, beta_rich, dec, iso)
                #beta_naf = ak.to_numpy(events['rich_beta_cor'])
                ...
                hist_mcbetareso[dec][iso].fill(true_ekin_atDec, beta_rich - true_beta_atDec, weights=mc_weight)
                hist_mcbetareso_mix[dec].fill(true_ekin_atDec, beta_rich - true_beta_atDec, weights=weight_mix)
...
def main():
    ...
    parser.add_argument("--resultdir", default="/home/manbing/Documents/Data/data_BeP8/RICHBeta", help="Directory to store plots and result files in.")
    ...
    for dec in detectors:
        hist_issbetareso[dec] = WeightedHistogram(xekinbinning, binning_inversebeta_residual[dec], labels=['Ekin/n (GeV/n)', r"$\mathrm{beta_{tof} - beta_{naf}}$"])
        hist_mcbetareso_mix[dec] = WeightedHistogram(xekinbinning, binning_inversebeta_residual[dec], labels=['Ekin/n (GeV/n)', r"$\mathrm{1/\beta}$"])
        for iso in isotopes:
            hist_mcbetareso[dec][iso] = WeightedHistogram(xekinbinning, binning_inversebeta_residual[dec], labels=['Ekin/n (GeV/n)', r"$\mathrm{1/\beta}$"])
    ...
    np.savez(os.path.join(args.resultdir, f"{nuclei}_richbetaResidual_refTrueBetaAtRICH_B1236P8_mcweight_finebinV2.npz"), **result_resolution)
