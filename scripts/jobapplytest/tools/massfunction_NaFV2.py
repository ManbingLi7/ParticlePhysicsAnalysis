...
class NaFInverseMassFunctionFit(InverseMassFunctionFit):
    ...
    def make_mass_function_simultaneous(self, drawiso="All"):
        ...
        if self.is_constraint:
            parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc', 'fraccore', 'sigma_ratio', 'asy_factor' ] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in isotopes_atom_num[:-1] for ibin in range(num_energybin)]
        else:
            parnames = ['x', 'mua', 'mub', 'muc', 'siga', 'sigb', 'sigc', 'fraccore', 'sigma_ratio', 'asy_factor' ] + [f'ex{a}_{iso}' for iso in self.isotopes[1:] for a in ["a", "b", "c"]] +  [f"n{isonum}_{ibin}" for isonum in isotopes_atom_num  for ibin in range(num_energybin)]
    ...
    def perform_fit(self, guess, fit_simultaneous=True, verbose=False, fixed_pars=None, lim_pars=None, parlim=None):
        ...
        if self.nonlinearconstraint and (parlim is not None):
            ...
            constraint_sig = NonlinearConstraint(lambda *par: poly(np.log(x_energy), par[3], par[4], par[5]), parlim['sigma'][0, :], parlim['sigma'][1, :])
            #print('upper-init: ', parlim['sigma'][1, :]  - poly(np.log(x_energy), guess['siga'], guess['sigb'], guess['sigc'], guess['sigd']))
            #print('init-lower: ', poly(np.log(x_energy), guess['siga'], guess['sigb'], guess['sigc'], guess['sigd']) - parlim['sigma'][0, :])
            constraint_fraccore = NonlinearConstraint(lambda *par: poly(np.log(x_energy), par[6]), parlim['fraccore'][0, :], parlim['fraccore'][1, :])
            constraint_sigmaratio = NonlinearConstraint(lambda *par: poly(np.log(x_energy), par[7]), parlim['sigma_ratio'][0, :], parlim['sigma_ratio'][1, :])
            constraint_asy_factor = NonlinearConstraint(lambda *par: poly(np.log(x_energy), par[8]), parlim['asy_factor'][0, :], parlim['asy_factor'][1, :])
            all_constraints = [constraint_sig, constraint_fraccore,  constraint_sigmaratio, constraint_asy_factor]
