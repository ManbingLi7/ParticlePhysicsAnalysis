...
def plot_fitmc_compare_isopars(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    ...
    for dec in detectors:
        ...
        ratio_b10b11[dec] = graph_init[dec]["B11"]/graph_init[dec]["B10"]
        ratio_b10b11[dec].add_to_file(df_sigma_ratio, f'graph_mass_sigma_7to6_{dec}')
        
        plot_graph(fig, ax2, ratio_b10b11[dec], color=ISOTOPES_COLOR['B11'], style="EP", xlog=True, ylog=False, scale=None, markersize=22, label='B11/B10')
        if dec =="Tof":
    ...
    if parname == "mean" or parname == "sigma":
        ...
        #ax2.axhline(y=ISOTOPES_MASS['B10']/ISOTOPES_MASS['B11'], color='green', linestyle='--')
        label_ref = mlines.Line2D([], [], linestyle='--', color=ISOTOPES_COLOR['B11'], label='ref m6/m7')
        legend2 = ax2.legend(handles=[label_ref], loc='upper left', bbox_to_anchor=(0.05, 1.02), fontsize=20)
        ax2.add_artist(legend2)

    if parname == "mean":
        label_com = mlines.Line2D([], [], linestyle='--', marker = "o",  markersize= 14, color=ISOTOPES_COLOR['B10'], label=r"$\mathrm{\mu_{7}}$/$\mathrm{\mu_{6}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)

    if parname == "sigma":
        label_com = mlines.Line2D([], [],  marker = "o",  markersize= 12, color=ISOTOPES_COLOR['B11'], label=r"$\mathrm{\sigma_{7}}$/$\mathrm{\sigma_{6}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)
...
def plot_fitmc_compare_isopars_rigreso(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    ...
    for dec in detectors:
        ...
        ratio_b11b10[dec] = graph_init[dec]["B11"]/graph_init[dec]["B10"]
        ratio_b11b10[dec].add_to_file(df_sigma_ratio, f'graph_mass_sigma_9to7_{dec}')
    
        plot_graph(fig, ax2, ratio_b11b10[dec], color=ISOTOPES_COLOR['B11'], style="EP", xlog=True, ylog=False, scale=None, markersize=22, label='B11/B10')
    ...
    ax1.legend(loc='center right', fontsize=FONTSIZE-2)  
    if parname == "mean":
        label_com = mlines.Line2D([], [], linestyle='--', marker = "o",  markersize= 12, color=ISOTOPES_COLOR['B11'], label=r"$\mathrm{\mu_{6}}$/$\mathrm{\mu_{7}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)
        
    if parname == "sigma":
        label_com = mlines.Line2D([], [],  marker = "o",  markersize= 11, color=ISOTOPES_COLOR['B11'], label=r"$\mathrm{\sigma_{7}}$/$\mathrm{\sigma_{6}}$")
        legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
        ax2.add_artist(legend3)
    ...
    ax1.legend(loc="lower right", fontsize=28)
    ax1.text(0.07, 0.98, f"B MC", fontsize=FONTSIZE, verticalalignment='top', horizontalalignment='left', transform=ax1.transAxes, color="black", fontweight="bold") 
    ax1.set_ylim(yrange[parname])
    ax1.fill_betweenx(np.linspace(yrange[parname][0], yrange[parname][1], 100), ANALYSIS_RANGE_RIG['B']['NaF'][0], ANALYSIS_RANGE_RIG['B']['NaF'][1], alpha=0.1, color=DETECTOR_COLOR['NaF'])  
    plt.subplots_adjust(hspace=.0)
    #ax1.get_yticklabels()[0].set_visible(False)
    ax1.set_xticklabels([])
    set_plot_defaultstyle(ax1)
...
def plot_fitmc_compare_isopars_betareso(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    ...
    for dec in detectors:
        ...
        ratio_b11b10[dec] = graph_init[dec]["B11"]/graph_init[dec]["B10"]
        ratio_b11b10[dec].add_to_file(df_sigma_ratio, f'graph_mass_sigma_7to6_{dec}')
    
        plot_graph(fig, ax2, ratio_b11b10[dec], color=ISOTOPES_COLOR['B11'], style="EP", xlog=True, ylog=False, scale=None, markersize=22, label='B11/B10')

        ax1.legend(loc='center right', fontsize=FONTSIZE-2)  
        if parname == "mean":
            label_com = mlines.Line2D([], [], linestyle='--', marker = "o",  markersize= 12, color=ISOTOPES_COLOR['B11'], label=r"$\mathrm{\mu_{7}}$/$\mathrm{\mu_{6}}$")
            legend3 = ax2.legend(handles=[label_com], loc='upper right', bbox_to_anchor=(0.9, 1.02), fontsize=20)   
            ax2.add_artist(legend3)
