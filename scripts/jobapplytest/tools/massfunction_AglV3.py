...
class LeastSquareMassFit:
    ...
    def __call__(self, *pars):
        ...
        sigma_ratio_amc, sigma_ratio_bmc, sigma_ratio_cmc = self.pmc[10:13]
        asy_factor_amc, asy_factor_bmc, asy_factor_cmc = self.pmc[13:16]
        
        sigma_guess = poly(np.log(energy), siga_mc, sigb_mc, sigc_mc, sigd_mc)
        sigma_err = 0.01 * sigma_guess
        
        fraccore_amc, fraccore_bmc, fraccore_cmc = self.pmc[7:10] 
        fraccore_guess =  poly(np.log(energy), fraccore_amc, fraccore_bmc, fraccore_cmc) 
