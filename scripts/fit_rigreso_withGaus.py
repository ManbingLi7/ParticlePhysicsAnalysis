...
from tools.constants import NUCLEI_CHARGE, ANALYSIS_RANGE_EKIN, ISOTOPES_MASS, ISOTOPES_COLOR, ISO_LABELS, NUCLEI_NUMBER
...
def main():
    ...
    parser.add_argument("--filename", default="/home/manbing/Documents/Data/jiahui/MC_Events/",   help="Path to root file to read tree from")
    ...
    parser.add_argument("--plotdir", default="/home/manbing/Documents/lithiumanalysis/scripts/plots/massfit/fitmc_tunedbeta", help="Directory to store plots and result files in.")    
    ...
    for iso in ISOTOPES[nuclei]:
        ...
        fit_mass_range[iso] = [1/(isonum*1.25), 1/(isonum * 0.87)]   
    ...
    for dec in detectors:
        ...
        for iso in ISOTOPES[nuclei]:
            ...
            for ibin in range(bin_range[0], bin_range[1]+1):
                ...
                guess = dict(mean=1/NUCLEI_NUMBER[iso], sigma=0.016/iso_mass_ratio[iso], norm=1000)
                #pars_guess = dict()
                #for par in dict_pars[temp].keys():
                #    pars_guess[par] = par_polynomials[par](np.log(kNucleiBinsRebin_center[ibin]))
                m = Minuit(loss, **guess)

                inverse_mass = 1/NUCLEI_NUMBER[iso]
        ...
        savefig_tofile(fig, args.plotdir, f"mean_compare_be10_{dec}", show=True)
