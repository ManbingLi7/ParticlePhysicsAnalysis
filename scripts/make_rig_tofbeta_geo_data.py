...
rich_selectors = {"LIP": {"Tof": selector_tof, "NaF": selector_naf_lipvar, "Agl": selector_agl_lipvar},
                  "CIEMAT": {"Tof":, "NaF": selector_naf_ciematvar, "Agl": selector_agl_ciematvar}}
...
def handle_file(arg):    
    ...
    for dec in detectors:
        ...
        hist_iss_tofbeta[dec] = Histogram(Binning(fbinning_beta()), labels=['Ekin/n (GeV/n)'])
        for iso in isotopes:
    ...
    for dec in detectors: 
        for events in read_tree(filename_iss, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
            ...
            hist_iss_tofbeta[dec].fill(beta_tof, ekin_rich)
