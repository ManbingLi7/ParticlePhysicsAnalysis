...
def power_law_v2(rigidity, phi, c, rigidity_scale, phi2, phi3, c2, c3, rigidity_scale2, rigidity_scale3, gamma1, gamma2, gamma3):
    effective_rigidity = rigidity + phi
    yfit = np.zeros_like(rigidity)
    yfit[rigidity < 8] = power_law(rigidity[rigidity < 8], phi, c, rigidity_scale, gamma1)
    yfit[(rigidity >= 8) & (rigidity <= 300)] = power_law(rigidity[(rigidity >= 8) & (rigidity <= 300)], phi2, c2, rigidity_scale2, gamma2)
    yfit[rigidity > 300] = power_law(rigidity[rigidity > 300], phi3, c3, rigidity_scale3, gamma3)
    return yfit


def fit_flux(flux_array, flux_title, guess=None, function=None, create_plot=False):
    ...
    if function is None:
        ...
        loss = LeastSquares(flux_array.rigidity, flux_array.flux, flux_error, model=function)
    m = Minuit(loss, **guess)
    m.migrad()
    ...
    def _parametrized_power_law(rigidity):
        return function(rigidity, **fit_param_dict)
...
def get_spline_from_graph(effgraph):
    spline_func = UnivariateSpline(effgraph.getx(), effgraph.gety(), w=1/(effgraph.get_yerrors()), k=3, s=5)
    return spline_func

def save_spline_to_file(spline_func, filedir, filename):
    filepath = os.path.join(filedir, filename)
    with open(filepath, 'wb') as f:
        pickle.dump(spline_func, f)   

def get_graph_from_spline(filepath, splinename, xvalues):
    with open(filepath, 'rb') as f: 
        spline_func = pickle.load(f) 
        yinterpolate_power_lawvalues = spline_func(xvalues)       
        graph_from_spline = MGraph(xvalues, yvalues)
    return graph_from_spline

def get_spline_from_file(filepath, splinename):
    with open(filepath, 'rb') as f: 
        spline_func = pickle.load(f) 
    return spline_func 
