...
def plot_fitmc_pars(parname, detectors, dict_fit_results_be7, massfit, poly_initp0, plotdir, ylabel, yrange, nuclei, isotope, figname=None, legend=None):
    ...
    df_pars_save = dict()
    par_ratio_range = {'mean': [0.978, 1.022], 'sigma':[0.962, 1.048], 'sigma_ratio': [0.902, 1.098], 'fraccore':[0.902, 1.098], 'asy_factor': [0.902, 1.098]}
    for dec in detectors:
        ...
        plot_graph(fig, ax1, graph_pars, color=DETECTOR_COLOR[dec],  style="EP", xlog=True, ylog=False, scale=None, markersize=26)
        ...
        ax1.fill_between(massfit[dec].x_fit_energy, yfit_lower, yfit_upper, color=DETECTOR_COLOR[dec], alpha=0.3)
        ax1.set_ylabel(ylabel[parname], fontsize=40)
        ...
        plot_graph(fig, ax2, graph_pull, color=DETECTOR_COLOR[dec], label=r'$\mathrm{\frac{y_{fit} - y_{d}}{\sqrt{e^{2}_{fit} + e^{2}_{d}}}}$', style="EP", xlog=True, ylog=False, scale=None, markersize=22)
        if parname == 'mean':
            ax1.grid(axis='y')
        ...
        plot_graph(fig, ax3, graph_ratio, color=DETECTOR_COLOR[dec], label='fit/data', style="EP", xlog=True, ylog=False, scale=None, markersize=22) 
        ...
        ax3.set_ylim(par_ratio_range[parname])
        ax2.grid(axis='y')
        ax2.set_xscale('log')        
        set_plot_defaultstyle(ax2)
...
def plot_fitdata_pars(parname, detectors, massfit, fit_parameters, polypars_initp0, ylabel, yrange, nuclei, plotdir, guess_initial, plot_mc=False, plot_guess=True, figname=None, guesserr=None, OptIso=None):
    parname_s = {'Tof': {'mean': 'mua', 'sigma':'siga' , 'fraccore': 'fraccore_a', 'asy_factor': 'asy_factor_a', 'sigma_ratio': 'sigma_ratio_a'},
    ...
    for dec in detectors:
        ...
        if plot_guess:
            print(guess_initial_values)
            ax1.plot(xval, yguess, '--', color='black', label=f'{dec} MC init')
            print(parname, pindex_s, pindex_e, yguess)

        
        #if guesserr is not None:
        ...
        graph_par[dec] = MGraph(xval, yfit, yfit_err)
        graph_par[dec].add_to_file(df_fitdata, f'graph_{dec}_{parname}')
    ...
    np.savez(os.path.join(plotdir, f'graph_fitpar_{parname}_{OptIso}.npz'), **df_fitdata)
...
def save_fitted_kscaler(massfit, fit_parameters):
    nuclei = massfit.nuclei
    isotopes = massfit.isotopes
    parname_s = [f'muscale_{iso}_a' for iso in isotopes[1:]] + [f'sigscale_{iso}_a' for iso in isotopes[1:]]
    print(parname_s)
    for par in parname_s:                                  
        yfit, yfit_err = get_fitpdf_witherr(np.log(xval), fit_parameters[dec][pindex_s:pindex_s+3], upoly) 
...
def plot_fitmc_compare_isopars_betareso_Be(parname, detectors, dict_fit_results, massfit, plotdir, poly_initp0, ylabel, yrange, nuclei, isotopes, legend=None):
    xaxistext = {"Tof": 0.03, "NaF": 0.33, "Agl": 0.75}
    ylim_compare = {"mean":[0.9, 1.1], "sigma": [0.9, 1.1], "fraccore": [0.9, 1.1], "sigma_ratio": [0.9, 1.1], "asy_factor": [0.9, 1.1]}
    ratio_be9be7 = dict()
    ratio_be10be7 = dict()
...
def savegraphpar(parname, detectors, dict_fit_results, massfit,nuclei, isotopes):
    ...
    for dec in detectors:
        for iso in isotopes: 
            y_init, y_init_err = get_unp_pars_binbybin(parname, dict_fit_results[iso][dec], massfit[iso][dec].num_energybin)
            graph_par[dec][iso] = MGraph(massfit[iso][dec].x_fit_energy, y_init, y_init_err)
            graph_par[dec][iso].add_to_file(df_sigma, f'graph_sigma_{dec}_{iso}')
    np.savez(os.path.join(plotdir, f"df_betareso_{parname}.npz"), **df_sigma)
