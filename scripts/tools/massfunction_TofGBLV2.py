...
class LeastSquareMassFit:
    ...
    def __call__(self, *pars):
        ...
        constraint = {}
        AglSpline_err = {}
        errorbar_precent = {'mean': 0.01, 'sigma': 0.01, 'sigma_ratio': 0.02, 'fraccore': 0.02, 'asy_factor':0.02}
        ...
        if self.fitFreeP:
            ...
            if self.nuclei == 'Be':
                ...
                chisquare = np.sum((self.yvalues - yfit) ** 2 / self.yvalueserr ** 2) + constraint['mean'] + 5 * constraint['sigma']  + constraint['fraccore'] +   constraint['sigma_ratio'] + constraint['asy_factor']  +  constraint_muscale_all + constraint_sigscale_all
...
class TofInverseMassFunctionFit(InverseMassFunctionFit):
    ...
    def __init__(self, nuclei, isotopes, hist_data, fit_energy_range, fit_mass_range, detector, is_constraint, is_nonlinearconstraint=False, component="All"):                                  
        InverseMassFunctionFit.__init__(self, nuclei, isotopes, hist_data, fit_energy_range, fit_mass_range, detector, is_constraint, is_nonlinearconstraint=is_nonlinearconstraint, component="All")        
