...
#def add_points_to_graph(agraph, arrx, arry, arryerr):
#    agraph.xvalues =  np.concatenate((agraph.xvalues, arrx))
#    agraph.yvalues =  np.concatenate((agraph.yvalues, arry))
#    agraph.yerrs =  np.concatenate((agraph.yerrs, arryerr))
#    return agraph
...
def plot_graph(figure, plot, graph, color=None, label=None, style="EP", xlog=False, ylog=False, scale=None, shade_errors=False, adjust_limits=None, adjust_limits_x=None, adjust_limits_y=None, flip_axes=False, override_limits=False, draw_zeros=True, adjust_figure=False,  setscilabelx=False, setscilabely=False, tick_length=14, tick_width=1.5, tick_labelsize = 35 , xbinning=None,**kwargs):     
    ...
    if style == "EP":
        ...
        if xbinning is None:
            raise ValueError('No xbinning available')
        else:
            bin_indices = xbinning.get_indices(values_x)
            print()
            plot.step(xbinning.edges[bin_indices[0]-1:bin_indices[-1]+2], np.concatenate(([0], values_y, [0])), where="post", color=color, label=label, **kwargs) 
...
def Add_point_to_graph(agraph, x, y, yerr):
    agraph.xvalues = np.append(agraph.xvalues, x)
    agraph.yvalues = np.append(agraph.yvalues, y)
    agraph.yerrs = np.append(agraph.yerrs, yerr)
    return agraph
